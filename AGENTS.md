...
## Service Entry Methods — convenção (ARCH-13 do audit 2026-05-18)

Padrão único pra **services puros** (input → output, command pattern):
...
**Use `.call` por padrão** em todo service novo. Exceções aceitáveis (semântica explícita):

- `.ensure!` — idempotent find_or_create (`AgentBotIdentity.ensure!`, `BeaResolver.ensure!`)
- `.for_<context>` — lookup contextualizado (`ConfigResolver.for(account)`)
- `.track(event, payload)` — telemetria estilo DSL (`Telemetry.track('event_name', ...)`)

Convenções herdadas que **NÃO** são padrão:
- `.dispatch`, `.resolve`, `.build`, `.execute`, `.process` — todas equivalentes a `.call` no fluxo; preferir `.call`. Se já existirem callers usando, adicionar `alias_method :old_name, :call` em vez de renomear.

Services com **state interno multi-step** (lifecycle complexo) escapam dessa regra — usam `new(...)` + métodos públicos múltiplos (ex: `ChatService.new(account:, ...).respond(msg)`, `Llm::ConversationContext.new(...).remember(:key, val)`). Não tentar forçar `.call` quando o service tem mais de uma operação pública distinta.
...
- **i18n keys**: nest custom strings under a `BECLINIC.*` root in `en.json` / `en.yml` to keep them separate from upstream Chatwoot keys. **Plugin-scoped strings (FE-16/17 do audit 2026-05-18)**: cada plugin tem seu próprio `config/locales/pt_BR.yml` (público clínica BR é PT-BR). Namespace = `<plugin_name>.<area>.<key>` (ex: `internal_chat.controllers.messages.edit_window_expired`). Migrar incrementalmente — começar por strings user-facing (errors 4xx, system messages, push notifications); deixar comentários internos e logs em PT-BR ASCII. **LLM-generated text fica fora** (Bea responde via system prompt PT-BR; tradução automática exigiria decisão de produto). Frontend: `frontend/i18n/locale/pt_BR/*.json` (padrão Chatwoot), namespace `INTERNAL_CHAT.*` / `AI_AGENT.*`.
