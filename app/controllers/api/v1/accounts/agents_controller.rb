class Api::V1::Accounts::AgentsController < Api::V1::Accounts::BaseController
  ...
  def account_user_attributes
    [:role, :availability, :auto_offline, :is_agenda_provider]
  end

  def allowed_agent_params
    [:name, :email, :role, :availability, :auto_offline, :is_agenda_provider]
  end
  ...
  def new_agent_params
    params.require(:agent).permit(:email, :name, :role, :availability, :auto_offline, :is_agenda_provider)
  end

  def agents
    @agents ||= Current.account.users.order_by_full_name.includes(
      { account_users: :klivy_role }, { avatar_attachment: [:blob] }
    )
  end
