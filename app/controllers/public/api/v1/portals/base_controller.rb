class Public::Api::V1::Portals::BaseController < PublicController
  ...
  def set_locale(&action_block)
    switch_locale_with_portal(&action_block) if params[:locale].present?
    switch_locale_with_article(&action_block) if params[:article_slug].present?

    action_block.call
  end

  def switch_locale_with_portal(&action_block)
    @locale = validate_and_get_locale(params[:locale])

    I18n.with_locale(@locale) do |*_args|
      action_block.call
    end
  end

  def switch_locale_with_article(&action_block)
