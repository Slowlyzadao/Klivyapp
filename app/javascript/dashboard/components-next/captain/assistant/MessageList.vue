...
const isErrorMessage = sender => sender === 'error';
...
const getAvatarName = sender => {
  if (isUserMessage(sender)) return t('CAPTAIN.PLAYGROUND.USER');
  if (isErrorMessage(sender)) return '!';
  return t('CAPTAIN.PLAYGROUND.ASSISTANT');
};

const getMessageStyle = sender => {
  if (isUserMessage(sender)) {
    return 'bg-n-solid-blue text-n-slate-12 rounded-br-sm rounded-bl-xl rounded-t-xl';
  }
  if (isErrorMessage(sender)) {
    return 'bg-n-ruby-3 text-n-ruby-11 border border-n-ruby-6 rounded-bl-sm rounded-br-xl rounded-t-xl';
  }
  return 'bg-n-solid-iris text-n-slate-12 rounded-bl-sm rounded-br-xl rounded-t-xl';
};
