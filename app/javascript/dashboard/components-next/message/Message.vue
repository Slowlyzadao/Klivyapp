...
/**
 * Detects if the message was sent from the user's phone (WhatsApp cell)
 * Used to differentiate phone-sent messages from bot messages
 */
const isFromPhone = computed(() => !!props.contentAttributes?.fromPhone);
...
const contextMenuEnabledOptions = computed(() => {
  ...
  return {
    ...
    delete:
      ...
      checkPermission('chat', 'delete_message'),
...
const avatarInfo = computed(() => {
  ...
  // If no sender, check if it's from phone
  if (!props.sender) {
    if (isFromPhone.value) {
      return {
        name: '',
        src: '',
        iconName: 'i-lucide-smartphone',
      };
    }
    return {
...
const avatarTooltip = computed(() => {
  ...
  if (isFromApi.value) {
    return 'Enviado via BEA (API)';
  }
  if (isFromPhone.value) {
    return `${t('CONVERSATION.SENT_BY')} ${t('CONVERSATION.PHONE')}`;
  }
