...
// Detecta se é mensagem de grupo via content_attributes
const isGroupMessage = computed(() => {
  return !!(
    contentAttributes?.value?.isGroup || contentAttributes?.value?.is_group
  );
});

// Mostra o nome do remetente apenas em grupos e para mensagens de usuários
const showSenderName = computed(() => {
  return (
    isGroupMessage.value && !isBotOrAgentMessage.value && !!sender?.value?.name
  );
});

const hasTransparentBg = computed(() => {
  const currentClass = getCurrentInstance()?.vnode?.props?.class;
  if (typeof currentClass === 'string')
    return currentClass.includes('bg-transparent');
  if (Array.isArray(currentClass))
    return currentClass.includes('bg-transparent');
  if (currentClass && typeof currentClass === 'object')
    return !!currentClass['bg-transparent'];
  return false;
});
</script>
...
<template>
  ...
  >
    ...
    <div
      v-if="showSenderName"
      class="group-sender-name text-[13px] font-semibold mb-1"
      :class="[hasTransparentBg ? 'px-2 pt-2' : '']"
