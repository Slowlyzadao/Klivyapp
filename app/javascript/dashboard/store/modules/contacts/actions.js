...
export const actions = {
  search: async (
    { commit },
    { search, page, sortAttr, label, append = false, pageSize = 15 }
  ) => {
    commit(types.SET_CONTACT_UI_FLAG, { isFetching: true });
    try {
      const {
        data: { payload, meta },
      } = await ContactAPI.search(search, page, sortAttr, label, pageSize);
      if (!append) {
  ...
  get: async (
    { commit },
    { page = 1, sortAttr, label, pageSize = 15 } = {}
  ) => {
    commit(types.SET_CONTACT_UI_FLAG, { isFetching: true });
    try {
      const {
        data: { payload, meta },
      } = await ContactAPI.get(page, sortAttr, label, pageSize);
      commit(types.CLEAR_CONTACTS);
  ...
  active: async ({ commit }, { page = 1, sortAttr, pageSize = 15 } = {}) => {
    commit(types.SET_CONTACT_UI_FLAG, { isFetching: true });
    try {
      const {
        data: { payload, meta },
      } = await ContactAPI.active(page, sortAttr, pageSize);
      commit(types.CLEAR_CONTACTS);
      commit(types.SET_CONTACTS, payload);
      commit(types.SET_CONTACT_META, meta);
      commit(types.SET_CONTACT_UI_FLAG, { isFetching: false });
  ...
  filter: async (
    { commit },
    { page = 1, sortAttr, queryPayload, resetState = true, pageSize = 15 } = {}
  ) => {
    commit(types.SET_CONTACT_UI_FLAG, { isFetching: true });
    try {
      const {
        data: { payload, meta },
      } = await ContactAPI.filter(page, sortAttr, queryPayload, pageSize);
      if (resetState) {
        commit(types.CLEAR_CONTACTS);
        commit(types.SET_CONTACTS, payload);
        commit(types.SET_CONTACT_META, meta);
