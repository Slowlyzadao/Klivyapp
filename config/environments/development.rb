Rails.application.configure do
  ...
  config.active_storage.default_url_options = { host: ENV.fetch('FRONTEND_URL', 'http://localhost:3000') }
  ...
  # DISABLED: verbose_query_logs gera stack traces para cada query e deixa o servidor lento em dev
  ...
  # Inside Docker with bind mounts on Windows/WSL2, polling via `listen` is configured via ENV vars.
  ...
  # 'info' é suficiente para dev local — 'debug' despeja SQL de cada request e trava o I/O
  ...
  # Bullet: desabilitado em dev local para evitar overhead de logging
  # Habilite manualmente se quiser auditar N+1 queries: Bullet.enable = true
