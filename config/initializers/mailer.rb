Rails.application.configure do
  ...
  # Usamos `:letter_opener_web` (em vez do clássico `:letter_opener`) porque ele
  # também grava em `/tmp/letter_opener` E expõe UI navegável em `/letter_opener`
  # — útil quando o email foi disparado de outro dispositivo (celular via tunnel).
  config.action_mailer.delivery_method = :letter_opener_web if Rails.env.development? && ENV['LETTER_OPENER']
