class Rack::Attack
  ...
  ##-----------------------------------------------##
  # `/webhooks/livekit/egress` é assinado por JWT HS256 (validado no
  # controller), mas se `LIVEKIT_API_SECRET` vazar, atacante pode forjar
  # webhooks válidos e cada um enfileira `TranscribeRecordingJob` —
  # custo $$ Whisper/Claude explode. LiveKit em retry agressivo pode mandar
  # ~10/seg/recording por curto período; 100/min/IP cobre o pior caso
  # legítimo (3 egress × pequenos retries simultâneos × picos paralelos).
  throttle('webhooks/livekit/egress/ip',
           limit: ENV.fetch('RATE_LIMIT_LIVEKIT_WEBHOOK_IP', '100').to_i, period: 1.minute) do |req|
    req.ip if req.path == '/webhooks/livekit/egress' && req.post?
  end
