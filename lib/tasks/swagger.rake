...
module SwaggerTaskActions
  ...
  def self.execute_build_plugins
    swagger_dir = Rails.root.join('swagger')
    index_yml_relative_path = 'plugins_index.yml'
    swagger_json_relative_path = 'plugins_swagger.json'
    ...
    Dir.chdir(swagger_dir) do
      ...
      puts 'Plugins Swagger build was successful.'
      puts "Generated #{swagger_dir.join(swagger_json_relative_path)}"
    end
  end
...
namespace :swagger do
  ...
  desc 'build combined plugins_swagger.json from all plugins'
  task build_plugins: :environment do
    SwaggerTaskActions.execute_build_plugins
  end
end
