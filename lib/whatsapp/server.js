...
// Teto do backoff. Após as MAX_RECONNECT_ATTEMPTS rápidas, seguimos tentando
// devagar (a cada MAX_RECONNECT_DELAY_MS) PRA SEMPRE enquanto houver
// credenciais salvas — pra sessão se autocurar quando a rede volta (notebook
// acordando do sleep), sem martelar o WhatsApp.
...
async function forwardMessageToChatwoot(session, messageData) {
  ...
  try {
    ...
    const response = await fetch(webhookUrl, {
      ...
      // 30s: o webhook do Chatwoot pode demorar em cold-start do Rails dev
      // (reload de classes após edição de código). Com 10s o bridge desistia e
      // logava "network timeout" mesmo o Rails processando e respondendo depois.
      timeout: 30000,
    });
    if (response.ok) {
    ...
    sock.ev.on('connection.update', async update => {
      ...
      if (connection === 'close') {
        ...
        if (UNRECOVERABLE_CODES.has(statusCode)) {
          // Backoff exponencial com TETO — e NUNCA desiste de vez enquanto
          // houver credenciais salvas. Antes a sessão era PAUSADA após
          // MAX_RECONNECT_ATTEMPTS ("reconecte via UI") e parava de receber
          // mensagem até reconectar na mão (não há Docker healthcheck local
          // pra ressuscitar, e o watchdog de liveness só checa sessões já
          // `connected`). Bug do notebook: dorme → rede cai → as 5 tentativas
          // rápidas estouram antes do WiFi voltar → sessão morta pra sempre.
          // Agora: rápido no começo, depois lento (MAX_RECONNECT_DELAY_MS) pra
          // sempre — autocura quando a rede volta, sem martelar o WhatsApp.
          const expStep = Math.min(session.reconnectAttempts - 1, 5);
          const delay = Math.min(
            BASE_RECONNECT_DELAY_MS * 2 ** expStep,
            MAX_RECONNECT_DELAY_MS
          );
          const slowMode = session.reconnectAttempts > MAX_RECONNECT_ATTEMPTS;
          console.log(
            `[${inboxId}] 🔄 Reconectando em ${(delay / 1000).toFixed(0)}s ` +
              `(tentativa ${session.reconnectAttempts}${slowMode ? ', modo lento' : `/${MAX_RECONNECT_ATTEMPTS}`})...`
          );
          session.lastQR = null;
          session.status = 'disconnected';
...
// Presence ("digitando..." / "parou"). A Bea chama no início do turno e via
// heartbeat (a cada ~8s) enquanto pensa, e 'paused' ao terminar — o WhatsApp
// expira o composing sozinho em ~10s. Best-effort: responde 200 mesmo
// desconectado / sem `to` — presença NUNCA deve quebrar o pipeline de resposta.
app.post('/sessions/:inboxId/presence', async (req, res) => {
  const inboxId = req.params.inboxId;
  const session = getSession(inboxId);
  const { to, state } = req.body || {};

  if (!session.sock || session.status !== 'connected' || !to) {
    return res.json({ success: false, skipped: true });
  }

  const presence = ['composing', 'paused', 'recording', 'available', 'unavailable'].includes(state)
    ? state
    : 'composing';

  try {
    await session.sock.sendPresenceUpdate(presence, to);
    return res.json({ success: true, state: presence });
  } catch (err) {
    console.error(`[${inboxId}] ⚠️ presence (${presence}) falhou:`, err.message);
