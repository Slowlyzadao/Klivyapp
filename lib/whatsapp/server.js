...
// Use the built-in `fetch` from Node 18+ (Node 25 in this project).
// `require('node-fetch')` returns the v3 ESM namespace object — calling it
// throws "fetch is not a function" and silently breaks every webhook to the
// Chatwoot backend. Keep using the global instead.
...
const sessionsBaseDir = path.resolve(__dirname, 'sessions');
const mediaPath = path.resolve(__dirname, 'media_cache');
  ...
  try {
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
      ...
      const processMsg = async msg => {
        ...
        // Ignora mensagens muito antigas (mais de 2h)
        const twoHoursAgo = Math.floor(Date.now() / 1000) - 2 * 60 * 60;
        if (msg.messageTimestamp < twoHoursAgo) return;
