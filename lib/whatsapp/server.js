...
// Permite que o backend (ex: AiAgent enquanto a Bea processa uma resposta)
// dispare o "digitando..." no WhatsApp do paciente. Baileys exige um
// `presenceSubscribe` antes do primeiro update direcionado para um JID novo
// — fazemos isso aqui para evitar mensagens silenciosamente ignoradas.
//
// Body: { to: "5511999...@s.whatsapp.net", state: "composing"|"paused"|"available" }
app.post('/sessions/:inboxId/presence', async (req, res) => {
  const inboxId = req.params.inboxId;
  const session = getSession(inboxId);
  if (!session.sock || session.status !== 'connected') {
    return res.status(503).json({ success: false, error: 'não conectado' });
  }
  const { to, state } = req.body || {};
  if (!to || !state) {
    return res
      .status(400)
      .json({ success: false, error: '"to" e "state" obrigatórios' });
  }
  try {
    if (state === 'composing' || state === 'recording') {
      try {
        await session.sock.presenceSubscribe(to);
      } catch (_) {
        /* subscribe é best-effort */
      }
    }
    await session.sock.sendPresenceUpdate(state, to);
    return res.json({ success: true });
  } catch (e) {
    console.warn(`[${inboxId}] presence ${state} para ${to} falhou:`, e.message);
