...
// Sprint K — a aba "Teleconsulta" reaproveita 100% do formulário de Consulta
// (mesmos campos, mesmas validações). A única diferença operacional é o flag
// `telemedicine` na definição, que dispara duas coisas:
//   1. ao trocar pra essa aba, o modal seta `telemedicine_enabled=true` no
//      newEvent (e `event_type='consultation'` — uma teleconsulta É uma
//      consulta, só acontece por vídeo);
//   2. no save, useAgendaCrud serializa o flag em custom_attributes.
//
// O campo `eventType` na definição existe pra dizer qual event_type real
// vai pro backend (já que TAB_DEFS["telemedicine"].value !== event_type).
const TAB_DEFS = [
  {
    value: 'consultation',
    label: 'Consulta',
    confirmKey: 'consulta',
    titleKey: 'Nova consulta',
  ...
  },
  {
    value: 'telemedicine',
    label: 'Teleconsulta',
    confirmKey: 'teleconsulta',
    titleKey: 'Nova teleconsulta',
    eventType: 'consultation',
    telemedicine: true,
  },
  {
    value: 'appointment',
    label: 'Compromisso',
    confirmKey: 'compromisso',
    titleKey: 'Novo compromisso',
  ...
  },
  {
    ...
    label: 'Bloqueio',
    confirmKey: 'bloqueio',
    titleKey: 'Novo bloqueio',
    eventType: 'agenda_block',
    telemedicine: false,
  },
];
...
export default {
  ...
  computed: {
    ...
    // Reaproveita o form de Consulta na aba Teleconsulta — a diferença é
    // só semântica (flag custom_attributes.telemedicine_enabled). Centraliza
    // a condição num computed pra v-if's e header icons não divergirem.
    isConsultationLike() {
      return (
        this.activeTab === 'consultation' || this.activeTab === 'telemedicine'
      );
    },
    // Nome do profissional selecionado — usado no banner da Teleconsulta.
    agentName() {
      const id = this.newEvent.user_id;
      if (!id) return '';
    ...
    treatmentOptionsForSelect() {
      return (this.treatmentOptions || []).map(tr => {
        ...
        // Label fica limpo — só o nome do serviço. Hint à direita combina
        // duração + preço (separados por ' · '). Preço aparece SÓ se > 0
        // pra não poluir com "R$ 0,00" em serviços sem custo.
        // Busca interna do FormSelect inclui badge, então digitar o ID acha.
        const hint = [durHint, priceHint].filter(Boolean).join(' · ');
        return {
          value: tr.name,
          label: tr.name,
          badge: tr.id || null,
    ...
    canConfirm() {
      ...
      if (this.isConsultationLike) {
        // Consulta E Teleconsulta exigem `patient_id` real. Defesa em
        // profundidade contra evento órfão (Consulta sem registro de
        // paciente no banco) — antes exigia só `selectedPatientName`
        // (string), o que aceitava ghost state.
        return (
          Boolean(this.newEvent.patient_id) && Boolean(this.newEvent.user_id)
        );
      }
    ...
    slotRange() {
      ...
      if (
        ...
        const enabled = settings.week_days.filter(
  ...
  watch: {
    show(val) {
      if (val) {
        // Reabre a aba certa quando o modal é mostrado (criar ou editar).
        // Pra edição de evento existente com telemedicine_enabled=true,
        // queremos cair direto na aba "Teleconsulta" — não na "Consulta".
        this.activeTab = this.computeInitialTab();
        this.fetchPatients('');
    ...
    activeTab(newVal) {
      ...
      // entre Consulta, Teleconsulta, Compromisso e Bloqueio sem perder o
      // que já preencheu (paciente selecionado, título digitado, telefone,
      // contato de origem). Antes (≤ 1.5.5.13) o watch zerava patient_id
      // e selectedPatientName ao sair de Consulta — destrutivo.
      //
      ...
      // e o backend trata como linkagem opcional.
      const def = TAB_DEFS.find(t => t.value === newVal) || TAB_DEFS[0];
      const update = {};
      // event_type real (pra backend) — 'telemedicine' não existe lá; vira 'consultation'.
      if (this.newEvent.event_type !== def.eventType)
        update.event_type = def.eventType;
      // Flag de telemedicina. Setamos explícito (true ou false) ao trocar
      // de aba: troca pra Consulta limpa o flag, troca pra Teleconsulta
      // marca. Sem isso, mudar de Teleconsulta → Consulta deixaria o flag
      // ativo no save.
      if (this.newEvent.telemedicine_enabled !== def.telemedicine) {
        update.telemedicine_enabled = def.telemedicine;
      }
      if (Object.keys(update).length) this.emitUpdate(update);

      // Sub-modal "+ Novo paciente" só faz sentido pra Consulta-like.
      // Reabertura manual via picker continua disponível quando o agente
      // voltar pra uma dessas tabs.
      if (def.eventType !== 'consultation') {
        this.showQuickPatientModal = false;
        this.quickPatientAutoOpened = false;
      }
    },
  ...
  methods: {
    // Sprint K — decide qual aba abrir baseado em event_type + flag de
    // telemedicina. event_type='consultation' + telemedicine_enabled=true
    // → aba Teleconsulta. Sem flag → aba Consulta. Compromisso/Bloqueio
    // são mapeados pelo value direto.
    computeInitialTab() {
      const et = this.newEvent.event_type || 'consultation';
      if (
        et === 'consultation' &&
        this.newEvent.telemedicine_enabled === true
      ) {
        return 'telemedicine';
      }
      return et;
    ...
    onTreatmentChange(name) {
      ...
      if (tr && typeof tr === 'object' && tr.duration_minutes) {
        ...
        const slotsNeeded = Math.max(
          1,
          Math.ceil(tr.duration_minutes / interval)
        );
        ...
        if (this.newEvent.time_start) {
          ...
          const firstFree = this.timeSlots.find(
            s => !this.slotBlockedReason(s)
          );
          if (firstFree) {
    ...
    openNewPatientModal(viaAuto = false) {
      ...
      const seedName =
        this.newEvent.selectedPatientName || this.patientSearch || '';
    ...
    async fetchPhoneSuggestions() {
      ...
      try {
        ...
        this.quickPatientPhoneOpen =
          this.quickPatientPhoneSuggestions.length > 0;
      } catch {
        // ignore
      }
    },
    ...
    formatDynamicCpfInput(e, key) {
      ...
      this.emitUpdate({
        custom_values: { ...this.newEvent.custom_values, [key]: val },
      });
    },
...
<template>
  <Teleport to="body">
    ...
    >
      ...
      >
        <div
          class="flex items-stretch w-full h-full sm:w-auto sm:h-auto sm:max-h-[calc(100vh-2rem)]"
          @click.stop
        >
          <!-- ── MAIN MODAL ── -->
          <div
          ...
          >
            <!-- Header -->
            <div
              class="flex items-center justify-between px-4 sm:px-6 border-b border-n-weak shrink-0 min-h-[60px] sm:min-h-[68px]"
            >
              <div class="flex items-center gap-3 min-w-0">
                <div
                  class="flex items-center justify-center w-9 h-9 rounded-xl bg-n-blue-2 text-n-blue-11 text-base shrink-0"
                >
                  <i
                    :class="
                      activeTab === 'agenda_block'
                        ? 'i-lucide-calendar-x'
                ...
                <div class="min-w-0">
                  <h3
                    class="text-base font-semibold text-n-slate-12 leading-tight truncate"
                  >
                    {{ headerTitle }}
                  </h3>
                  <span
                    v-if="isEditing"
                    class="text-xs text-n-slate-10 mt-0.5 block"
                  >
                    #{{ editingEventId }}
                  </span>
                </div>
            ...
            <div class="px-4 sm:px-6 pt-4 sm:pt-5 shrink-0">
              <div
                class="inline-flex p-1 bg-n-alpha-1 rounded-xl gap-1 w-full sm:w-auto"
              >
                <button
                  v-for="tab in tabs"
                  :key="tab.value"
            ...
            <!-- Body -->
            <div
              class="px-4 sm:px-6 py-5 flex-1 overflow-y-auto flex flex-col gap-5 modal-scroll"
            >
              <!-- ── CONSULTA / TELECONSULTA TAB ──
                   Teleconsulta reaproveita 100% do form de Consulta (Sprint K).
                   A única diferença é o flag custom_attributes.telemedicine_enabled
                   que useAgendaCrud serializa no save. -->
              <template v-if="isConsultationLike">
                <!-- Banner explicativo da teleconsulta -->
                <div
                  v-if="activeTab === 'telemedicine'"
                  class="flex items-start gap-3 p-3 rounded-xl bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/20"
                >
                  <i
                    class="i-lucide-video w-5 h-5 text-blue-600 dark:text-blue-400 mt-0.5 shrink-0"
                  />
                  <div class="text-sm text-blue-900 dark:text-blue-200">
                    <strong>Teleconsulta por vídeo.</strong>
                    Na hora marcada, o profissional
                    <em>{{ agentName || 'responsável' }}</em>
                    poderá entrar na sala direto pelo card do agendamento. O
                    paciente também recebe o link no portal.
                  </div>
                </div>
                ...
                <div class="form-group">
                  ...
                  <input
                    ...
                    :placeholder="
                      ...
                      'Será preenchido com o nome do paciente'
                    "
                ...
                <div data-patient-picker>
                    ...
                    >
                      ...
                      >
                        ...
                        <template v-if="patientSearch">
                          ...
                          <span class="font-semibold">{{ patientSearch }}</span>
                        </template>
                        <template v-else> Cadastrar novo paciente </template>
                      </button>
                      <div
                        v-if="patientList.length"
                        class="border-t border-n-weak"
                      >
                        <div
                          class="px-3 pt-2 pb-1 text-xs font-semibold text-n-slate-10 uppercase tracking-wider"
                        >
                          Pacientes
                        </div>
                        <button
                      ...
                      <div
                        v-else-if="patientSearch"
                        class="px-3 py-3 text-sm text-n-slate-10 italic text-center border-t border-n-weak"
                      >
                  ...
                  <div
                    v-else
                    class="border border-n-weak rounded-xl bg-n-alpha-1 px-4 py-3 flex items-center gap-3"
                  >
                    <span
                    ...
                    <div class="flex-1 min-w-0">
                      ...
                      <div class="text-xs text-n-slate-10 truncate">
                        Telefone:
                        {{
                          newEvent.selectedPatientPhone
                            ? formatPhoneBR(newEvent.selectedPatientPhone)
                            : 'Não informado'
                        }}
                      </div>
                    </div>
                ...
                >
                  ...
                  <FormSelect
                    ...
                    clearable
                    @change="
                      value => emitUpdate({ category_id: value || null })
                    "
                  />
                </div>
...
class="text-red-400 ml-0.5"
                    ...
                    <FormSelect
                      ...
                      auto-searchable
                      @change="
                        value => updateCustomValue(`attr_${attr.id}`, value)
                      "
                    />
                    <textarea
                      ...
                      @input="
                        e =>
                          updateCustomValue(`attr_${attr.id}`, e.target.value)
                      "
                    />
                    <input
                    ...
                    <input
                      ...
                      inputmode="tel"
                      @input="
                        e => formatDynamicPhoneInput(e, `attr_${attr.id}`)
                      "
                    />
                    <div v-else-if="attr.type === 'cpf'">
                      <input
                        ...
                        inputmode="numeric"
                        @input="
                          e => formatDynamicCpfInput(e, `attr_${attr.id}`)
                        "
                      />
                      <span v-if="!isDynamicCpfValid(attr)"
class="error-msg"
                        >CPF inválido</span>
                    </div>
                    <input
                ...
                <div
                  class="border-t border-n-weak pt-5 grid grid-cols-1 sm:grid-cols-2 gap-4"
                >
                  <div class="form-group">
                    <label class="form-label">
                ...
                <div class="border-t border-n-weak pt-5 flex flex-col gap-4">
                  ...
                  <div>
                    <div class="flex items-baseline justify-between mb-2">
                      ...
                      <span
                        class="text-sm font-semibold text-n-blue-11 tabular-nums"
                        >{{ rangeLabel }}</span>
                    </div>
                    <div
                      class="grid grid-cols-[repeat(auto-fill,minmax(58px,1fr))] gap-1.5"
                    >
                      <button
            ...
            >
              ...
              >
                ...
                {{
                  isDeleting
                    ? $t('AGENDA.MODAL.BTN_SAVING')
                    : $t('AGENDA.MODAL.BTN_DELETE')
                }}
              </button>
          ...
          <div
            ...
            :class="
              showQuickPatientModal
                ? 'w-full sm:w-[320px] rounded-none sm:rounded-r-2xl opacity-100'
                : 'w-0 sm:w-0 opacity-0 pointer-events-none'
            "
          >
            <div class="w-full sm:w-[320px] flex flex-col h-full">
              <div
                class="flex items-center justify-between px-5 border-b border-n-weak shrink-0 min-h-[60px] sm:min-h-[68px]"
              >
                <div class="flex items-center gap-2.5">
                  <div
                    class="flex items-center justify-center w-8 h-8 rounded-lg bg-green-500/10 text-green-400 text-sm"
                  >
              ...
              <div
                class="p-5 flex-1 flex flex-col gap-4 overflow-y-auto custom-scroll"
              >
                <div class="form-group">
                  ...
                  <input
                    v-model="quickPatient.name"
                    type="text"
                    class="form-input"
                    :placeholder="$t('AGENDA.MODAL.PLACEHOLDER_NAME')"
                  />
                </div>
                <div class="form-group">
                  <label class="form-label text-n-slate-10">
                  ...
                  <input
                    v-model="quickPatient.last_name"
                    type="text"
                    class="form-input"
                    :placeholder="$t('AGENDA.MODAL.PLACEHOLDER_LAST_NAME')"
                  />
                </div>
                <div class="form-group relative">
                  <label class="form-label text-n-slate-10">{{
                    $t('AGENDA.MODAL.FIELD_PHONE')
                  }}</label>
                  <input
                    ...
                    class="form-input"
                    maxlength="15"
                    :placeholder="$t('AGENDA.MODAL.PLACEHOLDER_PHONE')"
                    @input="formatQuickPatientPhone"
                    @blur="quickPatientPhoneOpen = false"
                  />
                  <div
                    v-if="
                      quickPatientPhoneOpen &&
                      quickPatientPhoneSuggestions.length
                    "
                    class="absolute z-50 w-full top-full mt-1 bg-n-solid-1 border border-n-weak rounded-lg shadow-lg p-1"
                  >
                    <div
                      class="px-3 py-1.5 text-sm font-semibold text-n-slate-10 uppercase tracking-wider border-b border-n-weak"
                    >
                    ...
                    >
                      ...
                      <span v-if="sug.phone" class="text-n-slate-10 text-sm">{{
                        sug.phone
                      }}</span>
                    </div>
                  </div>
              ...
              <div
                class="px-5 py-4 border-t border-n-weak shrink-0 flex items-center justify-end gap-3"
              >
                ...
                >
                  {{
                    isSubmittingPatient
                      ? $t('AGENDA.MODAL.BTN_SAVING') || 'Salvando...'
                      : $t('AGENDA.MODAL.BTN_ADD') || 'Adicionar'
                  }}
                </button>
              </div>
            </div>
