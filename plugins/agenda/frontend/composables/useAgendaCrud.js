...
export function useAgendaCrud({
  ...
  const openEditEvent = event => {
    ...
    if (event.agenda_service_id) {
      const current = store.getters['agendaServices/getServiceById'](
        event.agenda_service_id
      );
      treatmentName =
        current?.name ||
        event.agenda_service?.name ||
    ...
    newEvent.value = {
      ...
      selectedPatientAvatarUrl:
        event.custom_attributes?.patient_avatar_url || null,
      custom_values: {},
      // Sprint K — lê o flag JSONB pra que ao editar o modal já abra na
      // aba "Teleconsulta". `=== true` defensivo: JSONB pode vir como
      // string "true" em casos legados — preferimos negar o impreciso.
      telemedicine_enabled:
        event.custom_attributes?.telemedicine_enabled === true,
    };
  ...
  const saveEvent = async () => {
    ...
    try {
      const startsAtDate = new Date(
        `${newEvent.value.date}T${newEvent.value.time_start}`
      );
      let endsAtDate = new Date(
        `${newEvent.value.date}T${newEvent.value.time_end}`
      );
      ...
      const payload = {
        ...
        custom_attributes: {
          ...
          // Sprint K — flag canônico de teleconsulta. Reaproveitado pelo
          // PatientPortal::TelemedicineSession (backend) e pelo
          // TelemedicineJoinButton (frontend). Mandamos explícito (true/false)
          // em vez de só quando true pra que desmarcar a aba de Teleconsulta
          // num edit limpe o flag.
          telemedicine_enabled: newEvent.value.telemedicine_enabled === true,
