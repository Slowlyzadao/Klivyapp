    ...
    <div class="svc-header">
      <div class="svc-header__intro">
        ...
        <p class="section-sub-title">
          ...
          <br>
          <span class="svc-header__hint">
            <i class="i-lucide-info size-[12px]" />
            Preço e categoria DRE são configurados em
            <strong>Financeiro → Procedimentos</strong>.
          </span>
        </p>
      </div>
        ...
        <table class="svc-table">
          <thead>
            <tr>
              ...
              <th class="svc-th svc-th-financial">Financeiro</th>
          ...
          <tbody>
            ...
            >
              ...
              <!-- Status Financeiro (cross-link com Financeiro > Procedimentos)
                   Mostra preço + categoria DRE se configurados; CTA "Configurar"
                   se não. Clique em qualquer estado leva pra Financeiro. -->
              <td class="svc-td svc-td-financial">
                <template v-if="pricingForService(service.id)">
                  <Tooltip
                    :label="isFreeService(pricingForService(service.id))
                      ? `Serviço gratuito (preço = R$ 0 intencional). Categoria DRE: ${pricingForService(service.id).category_name || '—'}`
                      : `Categoria DRE: ${pricingForService(service.id).category_name || '—'} · clique pra ajustar`"
                    position="top"
                    multiline
                  >
                    <button
                      type="button"
                      class="svc-fin-pill"
                      :class="isFreeService(pricingForService(service.id)) ? 'svc-fin-pill--free' : 'svc-fin-pill--ok'"
                      @click.stop="goToFinancialPricing(service.id)"
                    >
                      <i
                        :class="isFreeService(pricingForService(service.id))
                          ? 'i-lucide-gift'
                          : 'i-lucide-check-circle-2'"
                        class="size-[12px]"
                      />
                      <span>{{ formatPricingPrice(pricingForService(service.id)) || 'Configurado' }}</span>
                    </button>
                  </Tooltip>
                </template>
                <template v-else>
                  <Tooltip
                    label="Sem preço/categoria DRE — sem isso o serviço não vira receita no DRE"
                    position="top"
                    multiline
                  >
                    <button
                      type="button"
                      class="svc-fin-pill svc-fin-pill--missing"
                      @click.stop="goToFinancialPricing(service.id)"
                    >
                      <i class="i-lucide-alert-triangle size-[12px]" />
      ...
      >
        <div class="modal-box svc-modal-box">
          ...
          <div class="modal-body">
            ...
            <!-- Duração (largura completa). O Preço, que antes ficava ao lado
                 num grid, foi removido em 2026-05-22 e vive em
                 Financial::ServicePricing (/financial/v2/settings > Serviços). -->
            <div class="svc-field">
...
// FinancialV2 expõe `servicePricings.index()` que retorna [{agenda_service,
// pricing, stats}]. Usamos pra cruzar info financeira aqui sem duplicar CRUD.
...
// ── Status Financeiro (2026-05-23) ─────────────────────────────────
// Mapa agenda_service_id → { pricing, category_name } pra exibir uma
// coluna "Status Financeiro" que mostra se o serviço tem preço + categoria
// DRE configurados em Financeiro > Procedimentos. Reduz fricção pro
// operador descobrir quais serviços ainda precisam de configuração
// financeira sem precisar trocar de tela.
const pricingsByServiceId = ref({});
const pricingsLoading = ref(false);

async function loadPricings() {
  pricingsLoading.value = true;
  try {
    const { data } = await FinancialV2.servicePricings.index();
    const map = {};
    (data?.data || []).forEach(entry => {
      if (entry.agenda_service?.id && entry.pricing) {
        map[entry.agenda_service.id] = entry.pricing;
      }
    });
    pricingsByServiceId.value = map;
  } catch (e) {
    // Falha silenciosa — frontend cai pro fallback "Configurar" pra todos
    // (não bloqueia o uso normal de Serviços da Agenda).
    // eslint-disable-next-line no-console
    console.warn('[SettingsServices] loadPricings failed', e);
  } finally {
    pricingsLoading.value = false;
  }
}

function pricingForService(serviceId) {
  return pricingsByServiceId.value[serviceId] || null;
}

// Formata o preço particular como "R$ X.XXX" (compact). Casos:
//   - particular > 0          → "R$ X"
//   - particular=0, conv > 0  → "R$ Y (conv.)"
//   - ambos = 0 (intencional) → "Grátis"  ← pricing existe, é proposital
//   - pricing null            → caller mostra "Configurar"
// Distinção "Grátis" vs "Configurar" é importante: avaliação gratuita,
// revisão pós-cirúrgica, consulta de retorno são serviços oferecidos
// pela clínica que precisam aparecer como CONFIGURADOS, não pendentes.
function formatPricingPrice(pricing) {
  if (!pricing) return null;
  const particular = pricing.particular_price_cents || 0;
  const convenio = pricing.convenio_price_cents || 0;

  if (particular === 0 && convenio === 0) return 'Grátis';

  const cents = particular || convenio;
  const reais = cents / 100;
  const formatted = reais.toLocaleString('pt-BR', { minimumFractionDigits: 0, maximumFractionDigits: 0 });
  if (particular === 0 && convenio > 0) {
    return `R$ ${formatted} (conv.)`;
  }
  return `R$ ${formatted}`;
}

// Identifica visualmente o pill "Grátis" pra ter ícone diferente do
// preço normal (sparkles/gift em vez de check). Pequena melhoria de UX.
function isFreeService(pricing) {
  if (!pricing) return false;
  return (pricing.particular_price_cents || 0) === 0
      && (pricing.convenio_price_cents || 0) === 0;
}

// Deep-link pro Financeiro > Procedimentos com o service pré-selecionado
// via query param `?service_id=<id>`. A página de destino lê esse param e
// abre o modal de upsert do pricing direto.
const goToFinancialPricing = (serviceId) => {
  const accountId = route.params.accountId;
  router.push({
    name: 'financial_v2_settings_tab',
    params: { accountId, tab: 'services' },
...
// Mount: carrega a primeira página + pricings financeiros em paralelo.
onMounted(() => {
  fetchPage(1);
  loadPricings();
});

// Quando usuário sai/volta da página (ex.: configurou pricing em Financeiro
// e voltou), recarrega o mapa. `pageServices.length` muda quando a página
// renderiza — dispara reload silencioso pra refletir mudanças externas.
watch(() => route.fullPath, (newPath, oldPath) => {
  if (newPath !== oldPath && route.path.includes('/agenda/settings')) {
    loadPricings();
  }
});
...
.svc-root {
  ...
  /* Ritmo vertical unificado (era 24px + mb-3 na toolbar = ~36px até a
     tabela, gap exagerado). Agora 16px entre header, busca e tabela. */
  gap: 16px;
}
...
/* ── Status Financeiro pill (2026-05-23) ───────────────────────────
   Cross-link com Financeiro > Procedimentos. Verde quando preço +
   categoria DRE estão configurados, amber quando falta. Clique leva
   pra config financeira. */
.svc-fin-pill {
  display: inline-flex;
  align-items: center;
  gap: 5px;
...
.svc-fin-pill--ok:hover {
  background: rgba(16, 185, 129, 0.18);
  border-color: rgba(16, 185, 129, 0.4);
}
/* Variante "Grátis" — tom azul (não verde nem amber): comunica que é
   intencional (cortesia/lead generation), não erro de cadastro. */
.svc-fin-pill--free {
  background: rgba(59, 130, 246, 0.10);
  color: rgb(var(--blue-11));
  border-color: rgba(59, 130, 246, 0.28);
}
.svc-fin-pill--free:hover {
  background: rgba(59, 130, 246, 0.18);
  border-color: rgba(59, 130, 246, 0.4);
}
:root.dark .svc-fin-pill--free {
  background: rgba(59, 130, 246, 0.22);
  color: #93c5fd;
  border-color: rgba(59, 130, 246, 0.4);
}
.svc-fin-pill--missing {
  background: rgba(245, 158, 11, 0.10);
  color: #b45309;
  border-color: rgba(245, 158, 11, 0.32);
}
.svc-fin-pill--missing:hover {
  background: rgba(245, 158, 11, 0.18);
  border-color: rgba(245, 158, 11, 0.45);
}
:root.dark .svc-fin-pill--ok {
  background: rgba(16, 185, 129, 0.18);
  color: #6ee7b7;
  border-color: rgba(16, 185, 129, 0.4);
}
:root.dark .svc-fin-pill--missing {
  background: rgba(245, 158, 11, 0.20);
  color: #fcd34d;
  border-color: rgba(245, 158, 11, 0.45);
}

/* Hint no header da seção */
.svc-header__hint {
  display: inline-flex;
  align-items: center;
  gap: 5px;
...
.svc-header__hint strong { color: rgb(var(--slate-11)); font-weight: 600; }
.svc-th-actions {
  width: 1%;
  white-space: nowrap;
  text-align: right;
...
.svc-header {
  ...
  justify-content: space-between;
  gap: 16px;
  /* margin-bottom removido — o gap do .svc-root já cuida do espaçamento. */
  flex-wrap: wrap;
}
.svc-header__intro {
  flex: 1 1 320px;
