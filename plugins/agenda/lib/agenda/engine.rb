  ...
  class Engine < ::Rails::Engine
    ...
    config.to_prepare do
      ...
      Account.class_eval do
        ...
        # Bootstrap canon 2026-05-23: toda Account nova ganha automaticamente
        # 10 AgendaServices padrão (Clínica Geral, Endodontia, etc) espelhando
        # as categorias DRE L3 do Financial canon. Idempotente, async via job.
        #
        # Guard `defined?` evita falha durante boot/migrate (engine ainda não
        # carregou completamente). Erros logados mas não propagam — não quebra
        # Account.create se o bootstrap der ruim (operador resolve manualmente).
        after_create_commit do
          if defined?(::Agenda::BootstrapAccountJob)
            ::Agenda::BootstrapAccountJob.perform_later(id)
          elsif defined?(::Agenda::Bootstrap::SeedDefaultServices)
            # Fallback síncrono pra testes que não usam ActiveJob queue.
            ::Agenda::Bootstrap::SeedDefaultServices.call(account: self)
          end
        rescue StandardError => e
          Rails.logger.error("[Agenda bootstrap] Account ##{id} bootstrap failed: #{e.message}")
        end
      end
      ...
      Contact.class_eval do
        ...
        # dependent: :destroy (síncrono, NÃO async): a FK waiting_list_entries.
        # contact_id é restritiva e NOT NULL, então a deleção das entries
        # precisa acontecer na MESMA transação, antes do contato — destroy_async
        # estouraria a FK (o contato seria deletado antes do job dos filhos rodar).
        has_many :waiting_list_entries, dependent: :destroy, class_name: 'WaitingListEntry'
      end
    end
  end
end
