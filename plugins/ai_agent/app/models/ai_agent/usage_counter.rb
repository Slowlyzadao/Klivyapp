module AiAgent
  class UsageCounter < ApplicationRecord
    ...
    def self.monthly_total(account_id:, on: Date.current)
      where(account_id: account_id, date: on.beginning_of_month..on.end_of_month).sum(
        'input_tokens + output_tokens'
      )
    end

    def self.monthly_cost_cents(account_id:, on: Date.current)
      where(account_id: account_id, date: on.beginning_of_month..on.end_of_month).sum(:cost_cents)
    end
  end
end
