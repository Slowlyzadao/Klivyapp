module AiAgent
  module Emergency
    ...
    class Detector
      ...
      CLINICAL_PATTERNS = [
        # Sangramento ativo (qualifier obrigatório pra evitar
        # "tive um sangramentinho semana passada" virar emergência)
      ...
      def call
        ...
        return Match.new(category: :suicidal, keyword: $~.to_s) if SUICIDAL_PATTERNS.any? { |re| @text =~ re }
        return Match.new(category: :clinical, keyword: $~.to_s) if CLINICAL_PATTERNS.any? { |re| @text =~ re }
