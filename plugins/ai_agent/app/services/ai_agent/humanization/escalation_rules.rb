...
#
#   - the same user message arrived 5+ times in a row (loop, agent stuck)
#   - tool calls failed twice in a row
#
# NÃO escala por "frustração"/sentimento negativo: decisão do dono — a Bia
# resolve sozinha ou pergunta; transfer_to_human é só clínico/sensível,
# cadastral ou pedido explícito. (Sentimento ainda é rastreado pra telemetria
# e pro alerta de tom ofensivo, mas não dispara handoff.)
#
...
class AiAgent::Humanization::EscalationRules
  Decision = Struct.new(:escalate, :reason, keyword_init: true) do
    def escalate? = escalate
  end

  # NEGATIVE_THRESHOLD ainda é usado pelo tracking de sentimento em
  # ChatService#apply_sentiment (telemetria), mas NÃO dispara mais escalação.
  ...
  def evaluate
    ...
    # NÃO escala por sentimento negativo/frustração (decisão do dono).
