module AiAgent
  module InternalChat
    ...
    class Responder
      ...
      def self.respond(message_id:)
        new(message_id).respond
      end
      ...
      def message_mentions?(message, bea)
        ...
        # criada normalmente.
        message.mentions.where(ai_agent_id: bea.id).exists?
      end
      ...
      def generate_response(account, room, message, bea)
        ...
        AiAgent::InternalChat::Toolset.tools_for(account).each { |t| chat.with_tool(t) }
      ...
      def build_prompt(history, message)
        ...
        if replied
          sections << "Mensagem que está sendo respondida:\n#{replied.sender&.available_name || 'Alguém'}: #{replied.content.to_s.strip}"
        end
      ...
      def replied_message_for(message)
        ...
        ::InternalChat::Message.find_by(id: in_reply_to)
      end

      def model_for_account(account)
