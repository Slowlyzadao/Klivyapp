...
class AiAgent::Multimodal::ImageHandler
  ...
  VALID_CATEGORIES = %w[prescription clinical_photo exam_image document greeting other].freeze
  ...
  def classify
    ...
    @attachment.file.blob.open do |tempfile|
      ...
      match = raw[/\b(prescription|clinical_photo|exam_image|document|greeting|other)\b/]
  ...
  def classification_prompt
    <<~PROMPT
      ...
      - greeting      → imagem de SAUDAÇÃO/social, sem NADA clínico nem documento: "bom dia/boa tarde/boa noite/boa semana" com flores ou paisagem, frase motivacional, imagem religiosa (santo/Jesus/oração), meme, figurinha/sticker, corrente de WhatsApp
      - other         → qualquer outra que não se encaixe acima (selfie sem contexto, screenshot genérico, foto difícil de classificar)
  ...
  def build_result(category)
    ...
    when 'greeting'
      # Saudação/social (bom dia com flor, religioso, meme): responde com
      # carinho e NÃO escala — não enche o inbox da recepção. Sem staff_note.
      Result.new(
        category: category,
        patient_message: 'Aaah, que fofo! Obrigada pela mensagem 🌷 Como posso te ajudar hoje? 😊',
        staff_note: nil,
        priority: 'low',
