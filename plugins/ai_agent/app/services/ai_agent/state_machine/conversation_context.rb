...
class AiAgent::StateMachine::ConversationContext
  ...
  def offer_slot!(starts_at:, duration_minutes:, service_id:, service_name:,
                  ...
                  alternatives: [], ambiguous_reschedule: false)
    memory['pending_offer'] = {
      ...
      # True quando a listagem recente mostrou 2+ consultas: o "Sim" NÃO pode
      # agir sozinho (não sabemos QUAL consulta remarcar) — vai pro LLM.
      'ambiguous_reschedule' => ambiguous_reschedule,
  ...
  # Listagem recente mostrou 2+ consultas — não dá pra saber QUAL o paciente
  # quer remarcar/cancelar. O auto-book do "Sim" fica PROIBIDO (caso real:
  # paciente com 2 consultas pediu remarcação, "Sim" criou um TERCEIRO
  # agendamento em vez de remarcar).
  def remember_ambiguous_listing!
    memory['recent_listing_ambiguous_at'] = Time.current.to_i
    save!
  end

  def recent_listing_ambiguous?
    at = memory['recent_listing_ambiguous_at']
    at.present? && at.to_i >= (Time.current.to_i - 300)
  end
  ...
  def set_active_service!(id:, name:, period: nil)
