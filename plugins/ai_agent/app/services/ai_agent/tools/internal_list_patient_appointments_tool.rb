...
class AiAgent::Tools::InternalListPatientAppointmentsTool < AiAgent::Tools::BaseTool
  description <<~DESC
  ...
  def execute(patient_id:, upcoming_limit: 5, past_limit: 3)
    ...
    # SEC-12: read-only mas exige perm `agenda:view` (info de quem tem
    # consulta marcada não é pública internamente).
    unless invoking_user_can?(:agenda, :view)
      return {
        found: false,
        permission_denied: true,
        note_for_bea: 'Quem mencionou você não tem permissão pra ver a agenda. ' \
                      'Avise educadamente que essa info é restrita a quem tem acesso à agenda.'
      }
    end
