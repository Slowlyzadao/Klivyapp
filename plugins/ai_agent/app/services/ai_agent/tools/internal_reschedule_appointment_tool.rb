...
class AiAgent::Tools::InternalRescheduleAppointmentTool < AiAgent::Tools::BaseTool
  description <<~DESC
  ...
  def execute(agenda_event_id:, new_starts_at:, reason: '')
    ...
    # SEC-12: gate por perm `agenda:edit_event` do staff que invocou Bea.
    return permission_denied unless invoking_user_can?(:agenda, :edit_event)
  ...
  def permission_denied
    {
      ...
      note_for_bea: 'Quem mencionou você não tem permissão pra reagendar consultas. ' \
                    'Avise educadamente que só admins ou usuários com permissão de agenda podem reagendar.'
    }
  end
end
