...
const props = defineProps({
  ...
  // Tom visual: 'danger' (default — ruby, ação destrutiva) ou 'info' (azul,
  // prompt/orientação não-destrutiva, ex.: "configure o módulo"). Mantém o
  // componente reusável sem duplicar um segundo modal.
  tone: { type: String, default: 'danger' },
  // Sobrescreve o ícone padrão do tom (qualquer classe i-lucide-*).
  icon: { type: String, default: '' },
});

const TONE_ICONS = { danger: 'i-lucide-alert-triangle', info: 'i-lucide-settings-2' };
const resolvedIcon = computed(
  () => props.icon || TONE_ICONS[props.tone] || TONE_ICONS.danger,
);
const confirmColor = computed(() => (props.tone === 'info' ? 'blue' : 'ruby'));
...
<template>
  ...
  <Teleport to="body">
    ...
    >
      <div class="cdm-card">
        <div class="cdm-body">
          <div class="cdm-icon-wrap" :class="`cdm-icon-wrap--${tone}`">
            <i :class="[resolvedIcon, 'cdm-icon', `cdm-icon--${tone}`]" />
          </div>
          <div class="cdm-text">
...
/* Tom 'info' (não-destrutivo) — sobrescreve o vermelho default. Definido
   depois pra vencer por ordem (mesma especificidade). */
.cdm-icon-wrap--info { background: rgb(var(--blue-4)); }
.cdm-icon--info { color: rgb(var(--blue-11)); }
