...
/* Force the FormSelect trigger to match button height (32px). FormSelect default
   é 40px (vem da regra global `field-base h-10` do _base.scss do Chatwoot, vide
   [[feedback-input-global-field-base]]) — pagination usa botões de 32px e o
   select ficava desalinhado. Seletor real é `.ms-trigger` (Multiselect-style,
   não `.fs-trigger` que o fix anterior usou erradamente). `!important` é
   necessário porque a regra global tem specificidade 0,11,1. */
