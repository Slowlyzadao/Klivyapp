...
class Api::V1::Accounts::DocumentTemplatesController < Api::V1::Accounts::BaseController
  ...
  # GET .../document_templates/preview_values?patient_id=:id&professional_id=:id
  # Resolve TODAS as variáveis do catálogo pra um paciente real, pro preview
  # ao vivo do editor (mostra "Danilo Mamedes" no lugar do chip {{Nome}}).
  # `professional_id` é opcional — sem ele, usa o usuário logado (Current.user);
  # com ele, resolve professional.* pro profissional escolhido no preview.
  # Read-only: não toca em template nem gera PDF — só resolve valores.
  def preview_values
    authorize DocumentTemplate, :variables?

    patient = Current.account.patients.find(params[:patient_id])
    resolver = ::DocumentTemplates::Resolver.new(
      patient: patient,
      clinic: Current.account,
      professional: preview_professional
    )
    render json: { data: resolver.resolve_all }
  ...
  # Profissional do preview: o escolhido (professional_id, escopado aos agentes
  # da conta) ou o usuário logado quando nenhum é passado.
  def preview_professional
    return Current.user if params[:professional_id].blank?

    Current.account.users.find_by(id: params[:professional_id]) || Current.user
  end
