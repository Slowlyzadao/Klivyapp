  ...
  module Readers
    ...
    class ClinicReader
      ...
      def read(key)
        ...
        # Manual (tela "Dados da clinica" → custom_attributes) tem precedencia;
        # billing (Asaas) e o fallback automatico do cadastro de compra.
        when 'clinic.cnpj'         then attr('cnpj') || billing_customer&.cpf_cnpj
        ...
        when 'clinic.phone'   then attr('phone') || billing_customer&.phone
      ...
      # Cadastro de billing (Asaas) da clinica — fonte do CNPJ/telefone. Link
      # account -> Billing::Subscription -> asaas_customer_id -> Billing::Customer.
      # Defensivo (`defined?`) pra nao acoplar duro ao plugin billing.
      def billing_customer
        return @billing_customer if defined?(@billing_customer)

        @billing_customer =
          if defined?(::Billing::Subscription) && defined?(::Billing::Customer) && @account
            sub = ::Billing::Subscription.where(account_id: @account.id)
