...
module DocumentTemplates
  ...
  class Resolver
    ...
    # `inputs` — mapa { 'input.<chave>' => valor } com os campos de
    # preenchimento que o operador digitou na geração (ver InputDefinitions).
    def initialize(patient:, clinic:, professional:, now: Time.current, inputs: {})
      @patient = patient
      @clinic = clinic
      ...
      @readers = {
        ...
        # account passado pro ProfessionalReader achar o Financial::AgentProfile
        # (CRO + especialidades) do profissional NAQUELA conta.
        'professional' => Readers::ProfessionalReader.new(professional, clinic),
    ...
    def resolve(key, fallback: nil)
      # Campos de preenchimento: vêm do mapa `inputs` (digitados na geração),
      # não de cadastro. Aceita a chave cheia ('input.cid') ou só o sufixo.
      return resolve_input(key, fallback) if key.to_s.start_with?('input.')
    ...
    def resolve_input(key, fallback)
      value = @inputs[key].presence || @inputs[key.to_s.delete_prefix('input.')].presence
      value.present? ? value.to_s : safe_fallback(fallback)
    end
