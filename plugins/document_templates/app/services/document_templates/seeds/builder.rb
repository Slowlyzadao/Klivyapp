...
module DocumentTemplates
  module Seeds
    ...
    module Builder
      ...
      #
      # Strings VAZIAS são descartadas: virariam `{type:'text', text:''}`, que
      # o ProseMirror proíbe (RangeError "Empty text nodes are not allowed").
      # Um único text vazio faz o TipTap descartar o documento INTEIRO ao
      # carregar e o editor abre em branco. `paragraph('')` (linha em branco)
      # é o caso comum — vira parágrafo vazio (content: []), que é válido e
      # renderiza como uma linha em branco do mesmo jeito.
      def normalize_inline(content)
        Array(content).flatten.compact
                      .reject { |c| c.is_a?(String) && c.empty? }
                      .map { |c| c.is_a?(String) ? text(c) : c }
      end
    end
  end
end
