...
class DocumentTemplatesAPI extends ApiClient {
  ...
  // Resolve todas as variáveis pra um paciente real (preview ao vivo do editor).
  // professionalId é opcional (default = usuário logado no backend).
  previewValues(patientId, professionalId) {
    return axios.get(`${this.url}/preview_values`, {
      params: { patient_id: patientId, professional_id: professionalId || undefined },
    });
  }
