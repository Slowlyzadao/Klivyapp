        ...
        class BankAccountsController < BaseController
          ...
          def bank_params
            params.require(:bank_account).permit(
              ...
              # Campos do wireframe Setup #4 (2026-05-23):
              :cnpj, :pix_key, :cut_date, :color,
              :default_for_receivables, :default_for_payments
            )
          end
          ...
          def serialize(a)
            {
              ...
              estimated_balance_cents: a.estimated_balance_cents,
              active: a.active,
          ...
          # KPIs do wireframe: Contas Ativas, Saldo Inicial Total, Saldo Estimado Atual.
          def build_summary(serialized)
            actives = serialized.select { |a| a[:active] }
            {
              active_count: actives.size,
              total_count: serialized.size,
              initial_balance_total_cents: actives.sum { |a| a[:initial_balance_cents].to_i },
              estimated_balance_total_cents: actives.sum { |a| a[:estimated_balance_cents].to_i }
            }
          end
