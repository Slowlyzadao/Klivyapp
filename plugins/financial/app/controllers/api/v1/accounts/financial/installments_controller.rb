    ...
    module Accounts
      module Financial
        class InstallmentsController < BaseController
          ...
          # receber pagamento é rotina de balcão (recepção+); estornar reverte
          # dinheiro e exige gestor. Declarado ANTES do set_installment para
          # que o 403 preceda o 404 (não vaza existência da parcela).
          before_action :authorize_receive!, only: %i[pay]
          before_action :authorize_refund!,  only: %i[refund]
          ...
          def pay
            idempotent! do
              ...
              # Refactor 2026-05-24 (paridade com payment_receipts#create):
              # aceita `payment_method_id` (PaymentMethod específico de Settings)
              # + tipo/value pros modificadores (intenção do operador).
              pm_record = nil
              ...
              kind = pm_record&.kind || params[:payment_method] || @installment.payment_method

              modifiers = build_modifier_amounts(amount_cents)
          ...
          # Helper idêntico ao usado em payment_receipts_controller — recalcula
          # cents do gross × pct quando tipo='percent' (anti-tamper). Mantém
          # cents legacy quando frontend não enviar tipo+value.
          ...
          def refund
            idempotent! do
              ...
              # Refactor 2026-05-25: enriquece o `reason` com label do
              # PaymentMethod (canon Settings) quando `payment_method_id`
              # vier. Auditoria fica completa — "estornei via PIX Cielo".
              # Service interno continua usando método original do receipt
              # pra criar Entry (decisão arquitetural: refund preserva o
              # método de pagamento original do canon contábil).
              pm_label = nil
              if params[:payment_method_id].present?
                pm_record = ::Financial::PaymentMethod
                              .for_account(current_account.id).alive
                              .find_by(id: params[:payment_method_id])
                pm_label = pm_record&.name
              end
              pm_label ||= params[:payment_method].to_s.upcase.presence

              reason = [params[:notes], pm_label && "via #{pm_label}"]
          ...
          #
          # Auditoria 2026-05-22 (`ALTO-CTL-01`): antes não validava MIME type.
          # Attacker podia upload `.exe`/`.zip` disfarçado como "comprovante"
          # e usar URL signed pra distribuir malware (social engineering
          # intra-clínica). Agora whitelist explícita: apenas PDF/JPG/PNG/WEBP.
          PROOF_ALLOWED_CONTENT_TYPES = %w[
            application/pdf
            image/jpeg
            image/png
            image/webp
          ...
          def upload_proof
            ...
            # Validação de MIME type (server-side). Não confia no header do
            # cliente — Active Storage detecta o tipo real do conteúdo.
            content_type = file.respond_to?(:content_type) ? file.content_type : nil
            unless PROOF_ALLOWED_CONTENT_TYPES.include?(content_type)
              return render(json: {
                errors: ["Tipo de arquivo não permitido: #{content_type || 'desconhecido'}. Permitidos: PDF, JPG, PNG, WEBP."]
              }, status: :unprocessable_entity)
            end

            # Validação de tamanho (10MB limite).
            size = file.respond_to?(:size) ? file.size : (file.tempfile&.size if file.respond_to?(:tempfile))
            if size && size > PROOF_MAX_BYTES
              return render(json: {
                errors: ["Arquivo muito grande (#{(size / 1024.0 / 1024.0).round(1)}MB). Máximo: 10MB."]
              }, status: :unprocessable_entity)
            end
          ...
          # Receber pagamento: RECEPCAO/GERENTE/ADMIN. `require_role!` retorna
          # silencioso se autorizado e renderiza 403 (+ AuditLog 'denied') se não.
          def authorize_receive!
            require_role!('RECEPCAO', 'GERENTE', 'ADMIN')
          end

          # Estornar: reverte dinheiro e comissão — só gestor.
          ...
          def serialize(i, include_budget: false, include_receipts: false)
            ...
            breakdown = receipt_breakdown_for(i)
            ...
            data = {
              ...
              # Breakdown agregado dos recibos vinculados — usado pelo ícone
              # info + Tooltip moderno na lista (UX 2026-05-24). Permite o
              # operador ver de cara se o recebimento teve juros/multa/desconto
              # /crédito sem precisar abrir o recibo.
              has_modifiers: breakdown[:has_modifiers],
              is_partial_payment: breakdown[:is_partial],
              total_interest_cents: breakdown[:interest],
              total_fine_cents: breakdown[:fine],
              total_discount_cents: breakdown[:discount],
              total_credit_applied_cents: breakdown[:credit_applied],
              receipts_count: breakdown[:count]
            }
          ...
          # Soma os modificadores dos recibos vinculados. Quando há múltiplos
          # recibos (baixa parcial + finalização), os totais agregam tudo —
          # operador vê o impacto cumulativo na linha. `is_partial` reflete
          # se a soma dos itens cobre o `amount_cents` original.
          def receipt_breakdown_for(inst)
            items = inst.payment_receipt_items.includes(:receipt)
            receipts = items.map(&:receipt).compact.uniq

            interest = receipts.sum { |r| r.interest_amount_cents.to_i }
            fine     = receipts.sum { |r| r.fine_amount_cents.to_i }
            discount = receipts.sum { |r| r.discount_amount_cents.to_i }
            credit   = receipts.sum { |r| r.credit_applied_cents.to_i }
            applied  = items.sum { |it| it.amount_cents.to_i }

            {
              has_modifiers: (interest + fine + discount + credit).positive?,
              is_partial: applied.positive? && applied < inst.amount_cents,
              interest: interest,
              fine: fine,
              discount: discount,
