    ...
    module Accounts
      module Financial
        ...
        class ReportsController < BaseController
          ...
          #
          # Cache 5min (Fase 5 housekeeping). Cache key inclui `max(updated_at)`
          # de Entry/Expense/PaymentReceipt — qualquer mutação invalida
          # automaticamente sem precisar de bus de invalidação.
          #
          # Stampede control: `race_condition_ttl: 10` faz outras requisições
          # esperarem até 10s pelo refresh em vez de todas recalcularem juntas
          ...
          def dashboard
            ...
            cache_key = dashboard_cache_key(from: from, to: to, date: date)

            data = Rails.cache.fetch(cache_key, expires_in: 5.minutes, race_condition_ttl: 10) do
              if from && to
                ::Financial::Reports::DashboardKpis.call(account: current_account, from: from, to: to)
              else
                ::Financial::Reports::DashboardKpis.call(account: current_account, date: date)
              end
            end

            response.set_header('X-Cache-TTL', '300')
            render json: data
          end
          ...
          # GET /financial/v2/reports/top_procedures?from=&to=&limit=10
          # Top procedimentos no período — agrega BudgetItem por descrição,
          # somando qtd, total e calculando ticket médio. Default: mês corrente.
          # Usado no Dashboard (tabela "Top 10 Procedimentos").
          def top_procedures
            from = parse_date(params[:from], :start) || Date.current.beginning_of_month
            to   = parse_date(params[:to],   :end)   || Date.current.end_of_month
            limit = (params[:limit] || 10).to_i.clamp(1, 50)

            data = ::Financial::Reports::Charts.top_procedures(
              account: current_account, from: from, to: to, limit: limit
            )
            render json: data
          end

          # GET /financial/v2/reports/revenue_vs_goal?months=12
          # Receita mensal últimos N meses + meta TOTAL ativa do mesmo período.
          # Usado no Dashboard (chart "Receita vs Meta") — cor das barras varia
          # por % atingido (vermelho <60%, âmbar 60-100%, verde ≥100%).
          def revenue_vs_goal
            months = (params[:months] || 12).to_i.clamp(3, 24)
            render json: ::Financial::Reports::Charts.revenue_vs_goal(
              account: current_account, months: months
            )
          end
          ...
          # ── Hub Relatórios novos (2026-05-23) ─────────────────────────
          # 5 endpoints adicionados pro refator card-grid do Hub. Cada um
          # delega num service `Financial::Reports::*Report` próprio.

          # GET /financial/v2/reports/revenue_by_procedure?from=...&to=...
          def revenue_by_procedure
            require_role!('GERENTE', 'ADMIN', 'AUDITOR') and return unless user_has_any_role?(%w[GERENTE ADMIN AUDITOR])
            from = parse_date(params[:from], :start)
            to   = parse_date(params[:to],   :end)
            data = ::Financial::Reports::RevenueByProcedureReport.call(
              account: current_account, from: from, to: to
            )
            render json: data
          end

          # GET /financial/v2/reports/revenue_by_payment_method?from=...&to=...
          def revenue_by_payment_method
            require_role!('GERENTE', 'ADMIN', 'AUDITOR') and return unless user_has_any_role?(%w[GERENTE ADMIN AUDITOR])
            from = parse_date(params[:from], :start)
            to   = parse_date(params[:to],   :end)
            data = ::Financial::Reports::RevenueByPaymentMethodReport.call(
              account: current_account, from: from, to: to
            )
            render json: data
          end

          # GET /financial/v2/reports/revenue_by_professional_table?from=...&to=...
          # Sufixo `_table` evita conflito com o endpoint de chart (`revenue_by_professional`)
          # já existente em ReportsV2 do dashboard.
          def revenue_by_professional_table
            require_role!('GERENTE', 'ADMIN', 'AUDITOR') and return unless user_has_any_role?(%w[GERENTE ADMIN AUDITOR])
            from = parse_date(params[:from], :start)
            to   = parse_date(params[:to],   :end)
            data = ::Financial::Reports::RevenueByProfessionalReport.call(
              account: current_account, from: from, to: to
            )
            render json: data
          end

          # GET /financial/v2/reports/account_statement?bank_account_id=...&from=...&to=...
          def account_statement
            require_role!('GERENTE', 'ADMIN', 'AUDITOR') and return unless user_has_any_role?(%w[GERENTE ADMIN AUDITOR])
            return render(json: { error: 'bank_account_id obrigatório' }, status: :bad_request) if params[:bank_account_id].blank?

            from = parse_date(params[:from], :start)
            to   = parse_date(params[:to],   :end)
            data = ::Financial::Reports::AccountStatementReport.call(
          ...
          # GET /financial/v2/reports/goals_vs_actual?from=...&to=...
          def goals_vs_actual
            require_role!('GERENTE', 'ADMIN', 'AUDITOR') and return unless user_has_any_role?(%w[GERENTE ADMIN AUDITOR])
            from = parse_date(params[:from], :start)
            to   = parse_date(params[:to],   :end)
            data = ::Financial::Reports::GoalsVsActualReport.call(
              account: current_account, from: from, to: to
            )
            render json: data
          end
          ...
          # Cache key do dashboard incorpora `max(updated_at)` das tabelas que
          # alimentam KPIs. Qualquer create/update/delete em Entry, Expense ou
          # PaymentReceipt invalida automaticamente sem bus separado.
          # `Rails.cache.fetch` ainda respeita o TTL de 5min como fallback caso
          # o cálculo de `max_updated` vire stale.
          def dashboard_cache_key(from:, to:, date:)
            scope = [
              ::Financial::Entry.for_account(current_account.id).maximum(:updated_at),
              ::Financial::Expense.for_account(current_account.id).maximum(:updated_at),
              ::Financial::PaymentReceipt.for_account(current_account.id).maximum(:updated_at)
            ].compact.max.to_i

            [
              'financial/v2/dashboard',
              current_account.id,
              from&.to_s,
              to&.to_s,
              date.to_s,
