  ...
  class RefundPayment
    ...
    def call
      ...
      ActiveRecord::Base.transaction do
        ...
        # 4) Reverte CommissionEntry PROPORCIONALMENTE.
        #
        # Auditoria 2026-05-22 (`CRIT-SVC-05`): antes, qualquer estorno (mesmo
        # de R$ 1 de uma parcela R$ 1000) marcava TODA a CommissionEntry como
        # 'estornada' — zerava 100% da comissão. Agora:
        #
        # - Cria Financial::Refund com `refund_proportion_bps` (1..10000)
        # - Para cada CommissionEntry devida vinculada à parcela, cria uma
        #   NOVA entry com `commission_amount_cents` negativo proporcional
        #   ao bps + `reverses_commission_entry_id` apontando pra original
        # - A entry original NÃO muda (frozen_attributes); o reverso é uma
        #   nova linha contábil
        # - Em estorno integral (partial=false), bps=10000 = reverte 100%
        installments.each do |inst|
          # Quanto foi devolvido desta parcela específica?
          # Coluna FK é `financial_installment_id` (canon do projeto), não `installment_id`.
          item_for_inst = items.find { |i| i.financial_installment_id == inst.id }
          next unless item_for_inst

          item_refund_cents =
            if partial && total_items_cents.positive?
              (requested_refund * item_for_inst.amount_cents / total_items_cents.to_f).round
            else
              item_for_inst.amount_cents
            end

          # Cria Refund entity com snapshot da proporção
          proportion_bps =
            if item_for_inst.amount_cents.positive?
              (item_refund_cents.to_d / item_for_inst.amount_cents * 10_000).round.clamp(1, 10_000)
            else
              10_000
            end
          ...
          refund = ::Financial::Refund.create!(
            ...
            refund_amount_cents: item_refund_cents,
            refund_proportion_bps: proportion_bps,
            reason: (@reason.presence || 'Estorno sem motivo especificado'),
            refund_method: refund_method_for_entity,
            bank_account_id: @as_credit ? nil : @receipt.financial_bank_account_id,
            refunded_at: @refunded_at,
            reverses_payment_receipt_id: @receipt.id
          )

          # Cria CommissionEntry reverso proporcional pra cada comissão
          # devida/aprovada/paga vinculada a essa installment.
          inst.commission_entries
              .where(status: %w[devida aprovada a_pagar paga provisionada])
              .find_each do |original|
            partial_reversal = (original.commission_amount_cents * proportion_bps / 10_000.0).round

            ::Financial::CommissionEntry.create!(
              account_id: original.account_id,
              financial_installment_id: original.financial_installment_id,
              financial_budget_id: original.financial_budget_id,
              professional_id: original.professional_id,
              financial_commission_rule_id: original.financial_commission_rule_id,
              rule_snapshot: original.rule_snapshot.is_a?(Hash) ? original.rule_snapshot : {},
              base_amount_cents: original.base_amount_cents,
              mdr_deduction_cents: original.mdr_deduction_cents,
              lab_deduction_cents: original.lab_deduction_cents,
              calc_base_cents: original.calc_base_cents,
              commission_amount_cents: -partial_reversal,
              percent_basis_points: original.percent_basis_points,
              status: 'estornada',
              competence_date: @refunded_at,
              triggered_at: Time.current,
              reverses_commission_entry_id: original.id
            )
          end
        end
    ...
    # Mapeia método de devolução pra Refund.refund_method enum.
    # Default = bank_transfer; `@as_credit` força patient_credit.
    def refund_method_for_entity
      return 'patient_credit' if @as_credit

      case @receipt.payment_method.to_s
