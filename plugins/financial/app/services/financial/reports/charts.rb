module Financial
  module Reports
    ...
    module Charts
      ...
      # Top N procedimentos por descrição num período. Source: BudgetItem
      # de orçamentos APROVADOS/CONCLUIDOS (canon §4.3) — assim refletimos só
      # produção real, não rascunhos descartados.
      #
      # Retorna: [{ name, qty, total_cents, ticket_avg_cents }] ordenado por qty desc.
      def top_procedures(account:, from:, to:, limit: 10)
        from = from.to_date
        to   = to.to_date

        # `BudgetItem belongs_to :budget` (FK financial_budget_id, tabela
        # financial_budgets). JOIN usa o NOME DA ASSOCIATION (`:budget`),
        # condições usam o NOME DA TABELA (`financial_budgets`). Colunas
        # `quantity` e `total_cents` existem nas duas tabelas — qualificar
        # com `financial_budget_items.` evita PG::AmbiguousColumn.
        rows = Financial::BudgetItem
                 ...
                 .map do |name, qty, total|
                   {
                     ...
                     ticket_avg_cents: qty.to_i.positive? ? (total.to_f / qty).round.to_i : nil
                   }
                 end
                 .sort_by { |r| -r[:qty] }
                 .first(limit)

        { from: from.iso8601, to: to.iso8601, items: rows }
      end

      # Série mensal de receita realizada vs meta total ativa.
      # Pra cada mês dos últimos N: soma de Entry.amount com direction=in,
      # affects_dre=true, cash_date no mês. Meta = RevenueGoal kind=total
      # que cubra o mês.
      # Retorna: [{ month: "2026-05", revenue_cents, goal_cents | nil, pct_attained | nil }]
      def revenue_vs_goal(account:, months: 12)
        end_month = Date.current.beginning_of_month
        start_month = end_month - (months - 1).months

        # Loop mês a mês — N é pequeno (≤24), N+1 queries aceitáveis aqui.
        result = []
        (0...months).each do |i|
          m_start = start_month + i.months
          m_end   = m_start.end_of_month

          revenue = Financial::Entry
                      .for_account(account.id)
                      .for_dre.income
                      .on_cash_date(m_start, m_end)
                      .sum(:amount_cents)

          goal = Financial::RevenueGoal
                   .for_account(account.id)
                   .where(kind: 'total', metric: 'currency', active: true)
                   .where('start_date <= ? AND end_date >= ?', m_start, m_end)
                   .order(Arel.sql('(end_date - start_date) ASC'))
                   .first

          goal_cents = goal&.target_cents&.to_i
          pct = goal_cents && goal_cents.positive? ? ((revenue.to_f / goal_cents) * 100).round(1) : nil
