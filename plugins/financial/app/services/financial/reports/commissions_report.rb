module Financial
  module Reports
    ...
    class CommissionsReport
      ...
      # `trigger_event` da CommissionRule — quando a comissão dispara.
      # Mapeia o valor canônico (em snake_case) pra label curto pra coluna
      # "Gatilho" da UI. Compatível com Setup #5 (canon 2026-05-23).
      TRIGGER_LABELS = {
        'paciente_comparece'      => 'Paciente compareceu',
      ...
      def build_summary(all_entries, professionals)
        ...
        # KPIs novos (canon 2026-05-23) que o Dashboard de Comissões consome:
        # - total_generated_cents: tudo que VIROU comissão no período (devida +
        #   paga, sem estornadas) — independe de já ter Expense criada
        # - approved_a_pagar_cents: subset de `devida` que JÁ TEM Expense em
        #   A Pagar, ou seja, "aprovado pra pagamento" no fluxo Klivy
        # - active_rules_count: regras de comissão ativas hoje (sinal de
        #   saúde do setup, exibido como 4º KPI)
        generated = totals_aggregate[:due_cents] + totals_aggregate[:paid_cents]
        approved  = all_entries.select { |e| e.status == 'devida' && e.financial_expense_id.present? }
                              .sum(&:commission_amount_cents).to_i

        {
          total_generated_cents:    generated,
          ...
          approved_a_pagar_cents:   approved,
      ...
      def serialize_entry(e)
        ...
        # Wireframe pede coluna "PAPEL" (DR/SDR/Comercial) e "GATILHO"
        # (paciente_comparece/orcamento_aceito/etc). Ambos vêm da
        # CommissionRule snapshot via `belongs_to :commission_rule`.
        # Regras antigas (pré-Setup #5) podem ter role/trigger nil.
        role          = rule&.role
...

        {
          ...
          trigger_label: TRIGGER_LABELS[trigger_event] || trigger_event,
      ...
      def extract_procedure_name(budget)
        ...
        first_item&.description.presence || first_item&.treatment_item&.procedure_name.presence || first_item&.procedure_code.presence || budget.notes.presence
      end
    end
  end
end
