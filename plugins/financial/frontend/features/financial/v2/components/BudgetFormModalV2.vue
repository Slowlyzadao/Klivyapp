...
// Refactor 2026-05-24: PAYMENT_METHODS hardcoded substituído pela lista REAL
// configurada em Settings → Formas de Pagamento. Mesma centralização aplicada
// em todos os modais financeiros do projeto.
...
// Helper: resolve PaymentMethod pelo id (objeto completo).
function findPaymentMethod(id) {
  return paymentMethods.value.find(m => m.id === id) || null;
}
// Helper: deriva kind canon do id (compat com backend que ainda usa string).
function kindOf(id) {
...
// paymentMethodId substitui paymentMethod (string) — refactor 2026-05-24.
// Backend continua recebendo `payment_method` (kind) derivado via kindOf().
const paymentMethodId = ref(null);
const customInstallments = ref([]); // array of { amount_str, due_date, payment_method_id }
...
async function submit() {
  ...
  errorMessage.value = '';
  try {
    // Refactor 2026-05-24: deriva `payment_method` (kind canon string) do
    // PaymentMethod selecionado. Backend continua aceitando string legacy
    // mas também pode usar `payment_method_id` quando vier (compat dupla).
    const defaultKind = kindOf(paymentMethodId.value) || 'pix';
    ...
    if (approveAfter.value) {
      ...
      if (installmentsMode.value === 'custom') {
        approvePayload.installments_plan = customInstallments.value.map((i) => ({
          ...
          due_date: i.due_date,
          payment_method_id: i.payment_method_id,
          payment_method: kindOf(i.payment_method_id),
        }));
      }
...
function resetForm() {
  ...
  // PaymentMethodId será re-preenchido pelo loadPaymentMethods quando o
  // modal abrir de novo. Pré-seleciona PIX direto se já tiver lista.
  const pixDirect = paymentMethods.value.find(m => m.kind === 'pix' && !m.provider);
  paymentMethodId.value = pixDirect?.id || paymentMethods.value[0]?.id || null;
...
// Carrega payment_methods quando o modal abre (lista real de Settings).
watch(() => props.show, (val) => {
  if (val) loadPaymentMethods();
}, { immediate: true });
</script>
        ...
        <fieldset class="block">
          ...
          <div v-if="installmentsMode === 'uniform'" class="uniform-plan">
            <div class="row">
                ...
                >
                  ...
                  <template #option="{ option }">
                    <PaymentMethodBadge
                      ...
                      hide-installments
                    />
                    <span v-if="option.hint" class="bgt-pm-hint">
              ...
              <label>
                <span>1ª venc.</span>
                <DatePickerBR v-model="firstDueDate" placeholder="DD/MM/AAAA" />
              </label>
              <label>
            ...
            <table v-if="uniformInstallments.length" class="preview">
              ...
              <tbody>
                <tr v-for="i in uniformInstallments" :key="i.number">
                  ...
                  <td>
                    <PaymentMethodBadge
                      v-if="findPaymentMethod(i.payment_method_id)"
                      :kind="findPaymentMethod(i.payment_method_id).kind"
                      :method="findPaymentMethod(i.payment_method_id)"
                      size="sm"
                      hide-installments
                    />
                    <span v-else>—</span>
          ...
          <div v-else class="custom-plan">
            ...
            <div v-for="(ci, idx) in customInstallments" :key="idx" class="custom-row">
              ...
              <DatePickerBR v-model="ci.due_date" placeholder="DD/MM/AAAA" />
              <FormSelect
                v-model="ci.payment_method_id"
                :options="paymentMethodOptions"
                placeholder="Forma"
                auto-searchable
              >
        ...
        <fieldset class="block">
          ...
          <div class="row">
            ...
            <label>
              ...
              <DatePickerBR v-model="validUntil" placeholder="DD/MM/AAAA" />
            </label>
          </div>
          <div class="check">
            <Checkbox v-model="approveAfter">
...
/* Refactor 2026-05-24: hint do provedor à direita do option no dropdown
 * de PaymentMethod (mesmo padrão dos outros modais V2). */
.bgt-pm-hint {
  margin-left: auto;
...
/* Field do PaymentMethod precisa de min-width pro dropdown caber direito */
.bgt-pm-field { min-width: 180px; }
</style>
