...
async function fetchData() {
  ...
  try {
    ...
    professionals.value = (data?.professionals || []).slice(0, props.limit);
  } finally {
    loading.value = false;
  }
}
...
<template>
  <!-- Header interno removido: o card pai (`dash-v2__chart-card`) já tem
       título + subtítulo. Mantém só o gráfico e estados loading/empty. -->
  <div class="rprof-v2">
...
/* Wrapper sem background/border/padding — card pai já é o quadro. */
.rprof-v2 {
