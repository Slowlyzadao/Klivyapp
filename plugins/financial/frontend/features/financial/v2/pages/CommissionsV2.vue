...
// Capturados aqui (não no setup do hasFilter abaixo) pra garantir 1 só
// snapshot. Se o componente for re-montado num mês novo, recomputa.
...
// "A Pagar" não é um status real no canon — é um estado derivado: a
// CommissionEntry está `devida` E já tem `expense_id`. Visualmente usa
// tom blue (entre o amber de "devida" e o emerald de "paga"). Mantém os
// labels do wireframe sem mexer no canon dos status.
function effectiveStatusBadge(entry) {
  if (entry.status === 'devida' && entry.expense_id) {
    return { color: 'blue', icon: 'i-lucide-credit-card' };
  }
...
// "Limpar" só aparece quando o usuário SAIU dos defaults (mês corrente +
// todos os profissionais). Sem isso, o botão fica sempre visível porque
// os defaults `from`/`to` já preenchem o filtro. Capturei os defaults em
// closure pra evitar recálculo em cada acesso.
const defaultFrom = startOfMonthISO();
const defaultTo = endOfMonthISO();

const hasFilter = computed(() => {
  const f = filters.value;
  return !!f.professional_id || f.from !== defaultFrom || f.to !== defaultTo;
});
  ...
  <div class="finv2-page">
    <header class="finv2-page__header">
      <div class="finv2-page__header-text">
        ...
        <p class="finv2-page__subtitle finv2-page__subtitle--cycle">
          <span class="com-v2__cycle-step">Provisionada</span>
          <i class="i-lucide-arrow-right com-v2__cycle-arrow" />
          <span class="com-v2__cycle-step">Devida</span>
          <i class="i-lucide-arrow-right com-v2__cycle-arrow" />
          <span class="com-v2__cycle-step">A Pagar</span>
          <i class="i-lucide-arrow-right com-v2__cycle-arrow" />
          <span class="com-v2__cycle-step">Paga</span>
        </p>
      </div>
    </header>

    <div class="finv2-page__body">
      <!-- KPIs (canon 2026-05-23) — alinha visualmente com o ciclo do header:
           Gerado (todas as devida+paga) · Aprovado/A Pagar (devida com Expense)
           · Pago (paga) · Regras Ativas (saúde do setup) -->
      <div class="finv2-kpis">
        <div class="finv2-kpi">
          ...
          <div class="finv2-kpi__content">
            <span class="finv2-kpi__label">Total Gerado</span>
            <strong class="finv2-kpi__value">{{ centsToBRL(summary.total_generated_cents || 0) }}</strong>
            <span class="finv2-kpi__hint">
              {{ summary.entries_count || 0 }}
              {{ (summary.entries_count || 0) === 1 ? 'comissão' : 'comissões' }}
            </span>
          </div>
        </div>
        <div class="finv2-kpi">
          ...
          <div class="finv2-kpi__content">
            <span class="finv2-kpi__label">Aprovado / A Pagar</span>
            <strong class="finv2-kpi__value">{{ centsToBRL(summary.approved_a_pagar_cents || 0) }}</strong>
            <span class="finv2-kpi__hint">com Expense criada</span>
          </div>
        </div>
        <div class="finv2-kpi">
          ...
          <div class="finv2-kpi__content">
            <span class="finv2-kpi__label">Pagas</span>
        ...
        <div class="finv2-kpi">
          <div class="finv2-kpi__icon finv2-kpi__icon--neutral">
            <i class="i-lucide-percent w-4 h-4" />
          </div>
          <div class="finv2-kpi__content">
            <span class="finv2-kpi__label">Regras Ativas</span>
            <strong class="finv2-kpi__value">{{ summary.active_rules_count || 0 }}</strong>
            <span class="finv2-kpi__hint">vigentes hoje</span>
          </div>
        </div>
      </div>
                ...
                <table class="finv2-table">
                  ...
                  <tbody>
                    <tr v-for="e in prof.entries" :key="e.id">
                      <td>
                        <Badge
                          :label="e.status === 'devida' && e.expense_id ? 'A Pagar' : e.status_label"
                          :color="effectiveStatusBadge(e).color"
                          :icon="effectiveStatusBadge(e).icon"
                          size="xs"
                        />
                      </td>
                      <td>
                        <span v-if="e.role" class="com-v2__role-pill">{{ e.role }}</span>
                      ...
                      <td>
                        <Tooltip
                          v-if="e.trigger_event"
                          :label="`Evento canon: ${e.trigger_event}`"
                          position="top"
                        >
                          <span class="com-v2__trigger-pill">{{ e.trigger_label }}</span>
                        </Tooltip>
                        <span v-else class="finv2-table__td-muted">—</span>
                      </td>
                      ...
                      <td class="com-v2__td-actions">
                        <Tooltip v-if="e.status === 'devida' && !e.expense_id" label="Marcar como paga">
                          <BeclinicButton
                            size="xs"
                            variant="ghost"
                            color="blue"
...
/* Ciclo de status no subtítulo do header — pílulas finas alinhadas
   horizontal, com setas entre. Visualmente compacto, documenta o
   fluxo `provisionada → devida → "a pagar" → paga` sem ocupar área
   significativa. */
.finv2-page__subtitle--cycle {
...
/* "Papel" (DR/SDR/Comercial) — pílula pequena, mesmo estilo do badge
   ESPECIALISTA do UserPickerSelect. */
.com-v2__role-pill {
  display: inline-flex; align-items: center;
  padding: 2px 8px;
  border-radius: 4px;
...
:root.dark .com-v2__role-pill {
  background: rgba(59, 130, 246, 0.18);
  color: #93c5fd;
}

/* "Gatilho" — pílula neutra. Tooltip mostra o evento canon em
   snake_case ao passar mouse. */
.com-v2__trigger-pill {
  display: inline-flex; align-items: center;
  padding: 2px 8px;
  border-radius: 4px;
  font-size: 11px; font-weight: 500;
