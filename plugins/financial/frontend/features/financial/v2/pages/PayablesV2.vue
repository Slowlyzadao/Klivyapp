...
const STATUS_CHIPS = [
  ...
  { key: 'pendente',  label: 'Pendente',  tone: 'slate',   statuses: ['pendente'],            icon: 'i-lucide-calendar' },
...
async function reverseExpense(expense) {
  const reason = prompt('Motivo do estorno:');
  if (!reason) return;
  try {
    await FinancialV2.expenses.reverse(expense.id, { reason });
            ...
            <tr v-for="exp in expenses" v-else :key="exp.id">
              ...
              <td class="finv2-table__td-actions">
                <Tooltip v-if="isPayable(exp)" label="Pagar despesa">
                  <BeclinicButton size="xs" variant="faded" color="teal" icon="i-lucide-check" @click="payExpense(exp)" />
                </Tooltip>
      ...
      <div class="finv2-cards finv2-show-mobile">
        ...
        <article v-for="exp in expenses" v-else :key="`m-${exp.id}`" class="finv2-card">
          ...
          <div v-if="isPayable(exp) || exp.status === 'pago'" class="pyv2__card-actions">
            <BeclinicButton v-if="isPayable(exp)" size="sm" variant="faded" color="teal"
