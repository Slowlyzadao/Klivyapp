...
import { hasBreakdown, breakdownTooltipLabel } from '../composables/useInstallmentBreakdown';
import {
...
// Default `true` (esconde R$ 0) porque parcelas de serviço gratuito
// (avaliação inicial, revisão cortesia) poluem visualmente a lista
// de "a receber". Operador raramente precisa "dar baixa" em R$ 0 —
// elas só existem pra rastreamento DRE. Toggle UI permite mostrar
// quando precisar (auditoria, conferência).
const hideZeroInstallments = ref(true);
...
// `displayedGroups` aplica filtro client-side de "esconder R$ 0" sobre as
// parcelas de cada grupo. Quando o filtro esconde TODAS as parcelas de um
// paciente, o grupo inteiro é omitido pra não mostrar header vazio. Mantém
// o `groups` original intocado pra paginação/seleção do backend continuar
// referenciando o mesmo conjunto.
const displayedGroups = computed(() => {
  if (!hideZeroInstallments.value) return groups.value;
  return groups.value
    .map(g => ({
      ...g,
      installments: (g.installments || []).filter(i => (i.amount_cents || 0) > 0),
    }))
    .filter(g => g.installments.length > 0);
});

// Contagem total de parcelas "grátis" no resultado atual — usado pro hint
// do toggle ("X parcelas R$ 0 ocultas").
const hiddenZeroCount = computed(() => {
  return (groups.value || []).reduce((sum, g) => {
    return sum + (g.installments || []).filter(i => (i.amount_cents || 0) === 0).length;
  }, 0);
});
...
// ── Refund modal ─────────────────────────────────────────────────────
// Reusa o `RefundTransactionModal` da aba financeiro do paciente — mesmo
// fluxo (parcial/integral/crédito), mesmo backend
// (`POST /installments/:id/refund`). Aqui só plumbing: abre o modal com a
// parcela selecionada e despacha a chamada via `FinancialV2.installments.refund`
// direto (não usa o composable `useFinancialActions` que é específico do
// `patientId` do contexto da ficha do paciente).
const showRefundModal = ref(false);
const refundingInstId = ref(null);
const refundLoading = ref(false);

const refundingTx = computed(() => {
  const inst = allInstallments.value.find(i => i.id === refundingInstId.value);
  if (!inst) return null;
  // Adapter pro shape esperado pelo modal: `amount` em R$ (não centavos) e
  // `description` legível pro cabeçalho. Valor a estornar default = quanto
  // foi recebido (não o amount original — se já houve estorno parcial antes,
  // só sobra `received_amount_cents`).
  return {
    ...inst,
    amount: (inst.received_amount_cents || inst.amount_cents) / 100,
    description: inst.budget?.label || `Parcela ${inst.number}/${inst.total_in_series}`,
  };
});

function openRefundForRow(inst) {
  refundingInstId.value = inst.id;
  showRefundModal.value = true;
}

async function onRefundConfirm(payload) {
  if (!refundingInstId.value) return;
  refundLoading.value = true;
  try {
    await FinancialV2.installments.refund(refundingInstId.value, payload);
    notifySuccess('Estorno realizado.');
    showRefundModal.value = false;
    refundingInstId.value = null;
    await load();
  } catch (err) {
    // eslint-disable-next-line no-console
    console.error('[ReceivablesV2] refund error', err);
    const msg = err.response?.data?.errors?.join(', ') || 'Falha ao estornar transação.';
    notifyError(msg);
  } finally {
...
// Distinção entre 2 conceitos:
//   • `hasPayment` = parcela tem pagamento registrado (qualquer origem).
//     Determina se o botão de estorno DEVE APARECER na UI.
//   • `hasActiveReceipt` = parcela tem PaymentReceipt vinculado (fluxo v2).
//     Determina se o botão fica ATIVO ou DESABILITADO.
//
// Dados legados/migrados podem ter `status='recebido'` sem `PaymentReceiptItem`
// correspondente — o backend rejeita o refund ("Parcela não tem recibo de
// pagamento ativo"). Em vez de esconder o botão (deixaria o operador na
// dúvida), mostramos disabled com tooltip explicativo. Parcelas pagas pelo
// fluxo v2 sempre têm recibo (canon §5.3) — botão fica habilitado.
function hasPayment(inst) {
  const s = inst.effective_status || inst.status;
  return ['recebido', 'parcial'].includes(s) && (inst.received_amount_cents || 0) > 0;
}

function hasActiveReceipt(inst) {
  return (inst.receipts_count || 0) > 0;
}

function refundTooltipLabel(inst) {
  return hasActiveReceipt(inst)
    ? 'Estornar pagamento'
  ...
  <div class="finv2-page">
    ...
    <div class="finv2-page__body">
      ...
      <div class="rcv-v2__filters">
        ...
        <!-- Linha única (1.8.0.12): toggle "Esconder R$ 0" à esquerda +
             botão Expandir/Recolher à direita. Antes eram 2 linhas, dobrava
             altura visual sem ganho. Checkbox usa o componente padrão do
             design system (Checkbox de beclinic_core) — antes era input
             nativo com accent-color custom (visual desencontrado entre browsers). -->
        <div v-if="groups.length > 0" class="rcv-v2__list-toolbar">
          <Checkbox
            v-model="hideZeroInstallments"
            class="rcv-v2__zero-toggle"
          >
            <span class="rcv-v2__zero-toggle-text">
              Esconder parcelas R$ 0 (serviços gratuitos)
              <span v-if="hideZeroInstallments && hiddenZeroCount > 0" class="rcv-v2__zero-toggle-count">
                · {{ hiddenZeroCount }} {{ hiddenZeroCount === 1 ? 'oculta' : 'ocultas' }}
              </span>
            </span>
          </Checkbox>

          <!-- Quick actions: expandir/recolher todos -->
          <div class="rcv-v2__quick-actions">
            <button
              v-if="!allExpanded"
              type="button"
          ...
          >
                ...
                >
                  ...
                  >
                    ...
                    <td class="finv2-table__td-num finv2-table__td-num--strong">
                      <span class="rcv-v2__amount-wrap">
                        {{ centsToBRL(inst.remaining_cents) }}
                        <!-- Ícone info — aparece quando há juros/multa/desconto/crédito
                             aplicado OU pagamento parcial OU múltiplos recibos.
                             Tooltip mostra breakdown completo on-demand. -->
                        <Tooltip
                          ...
                          multiline
                        >
                          <i class="i-lucide-info rcv-v2__breakdown-icon" />
                        </Tooltip>
                      </span>
                      <span
                    ...
                    <td class="finv2-table__td-actions">
                      <!-- Cores `teal` (não blue) pra bater com o padrão da aba
                           Financeira do paciente (FinancialTimeline.vue:537,563)
                           — decisão UX 2026-05-28: ações de recebimento +
                           WhatsApp seguem mesma paleta nos 2 lugares. -->
                      ...
                      <Tooltip
                        v-if="hasPayment(inst)"
                        :label="refundTooltipLabel(inst)"
                        position="left"
                        multiline
                      >
                        <BeclinicButton
                          ...
                          color="ruby"
                          icon="i-lucide-undo"
                          :disabled="!hasActiveReceipt(inst)"
                          @click="hasActiveReceipt(inst) && openRefundForRow(inst)"
                        />
                      </Tooltip>
                    </td>
                  </tr>
            ...
            <div class="rcv-v2__inner-cards">
              ...
              >
                ...
                <div
                  v-if="isPayable(inst) || hasPayment(inst)"
                  class="rcv-v2__inner-card-actions"
                >
                  <BeclinicButton
                    v-if="isPayable(inst)"
                    size="sm"
...
/* Toolbar acima da lista — toggle "Esconder R$ 0" à esquerda, botão
   Expandir/Recolher à direita. Reduz altura visual (era 2 linhas). */
.rcv-v2__list-toolbar {
  display: flex; align-items: center; justify-content: space-between;
  gap: 12px; flex-wrap: wrap;
  padding: 4px 0;
}

/* Tipografia do label do Checkbox (componente Checkbox de beclinic_core
   tem font-size 14px; pra esse toggle queremos visual mais sutil pra
   não competir com o conteúdo principal da lista. Override via :deep). */
.rcv-v2__zero-toggle :deep(.bcl-checkbox__label) {
  font-size: 12.5px;
  color: rgb(var(--slate-11));
}
.rcv-v2__zero-toggle-text {
  display: inline-flex; align-items: center; gap: 6px;
}
.rcv-v2__zero-toggle-count {
  color: rgb(var(--slate-9));
...
/* Wrap pro valor + ícone info ficarem alinhados na mesma linha */
.rcv-v2__amount-wrap {
  display: inline-flex;
  align-items: center;
  gap: 6px;
...
/* Ícone info sutil — clique-livre, hover ativa Tooltip via Teleport */
.rcv-v2__breakdown-icon {
...
.rcv-v2__inner-card-actions {
  /* Mobile: cada botão estica pra ocupar largura igual (flex: 1) — em vez de
     ficarem encolhidos à direita. Tap target maior + visual mais polished
     no card view (decisão UX 2026-05-28). */
  ...
  /* `:deep` pra atravessar o scoped do BeclinicButton e Tooltip — força o
     <button> interno a expandir 100% da largura do wrapper flex. */
  :deep(button) {
    width: 100%;
...
/* Override scoped da coluna Ações — blindagem caso o partial `_layout.scss`
   não seja recompilado pelo HMR (mudança em partial às vezes não dispara
   rebuild do entry). Especificidade `[data-v-*]` + `!important` garante que
   essa regra vence qualquer outra. Mesmo conteúdo do `_layout.scss:402`. */
.rcv-v2__inner-table .finv2-table__td-actions {
  text-align: right !important;
  white-space: nowrap !important;
}
.rcv-v2__inner-table .finv2-table__th-actions {
  text-align: right !important;
}
.rcv-v2__inner-table .finv2-table__td-actions > * {
  vertical-align: middle;
