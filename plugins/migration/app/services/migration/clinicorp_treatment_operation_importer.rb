...
module Migration
  ...
  class ClinicorpTreatmentOperationImporter
    ...
    # Defaults aplicados a TreatmentItems criados a partir do CSV TreatmentOperation.
    # PR audit 2026-05-21: antes eram literais hardcoded no `treatment_item_attributes`
    # (priority: 'media', status: 'aprovado', discount_value: 0). Extraídos pra
    # constantes pra deixar claro que são DECISÕES DE PRODUTO, não bugs:
    #
    #   - DEFAULT_ITEM_PRIORITY = 'media' — fila normal (CSV não exporta
    #     prioridade); admin promove urgentes manualmente na UI.
    #   - DEFAULT_ITEM_STATUS = 'aprovado' — procedimentos do Clinicorp já
    #     passaram pelo workflow de aprovação lá, importam direto sem rever.
    #   - DEFAULT_ITEM_DISCOUNT_CENTS = 0 — CSV não traz desconto histórico;
    #     admin ajusta na mão se quiser preservar campanhas antigas.
    #
    # Pra cliente que precisar comportamento diferente: o jeito limpo é
    # parametrizar via initializer; o jeito rápido é editar essas constantes.
    DEFAULT_ITEM_PRIORITY        = 'media'.freeze
    DEFAULT_ITEM_STATUS          = 'aprovado'.freeze
    DEFAULT_ITEM_DISCOUNT_CENTS  = 0
    ...
    def treatment_item_attributes(plan_id, mapped)
      {
        ...
        discount_value: DEFAULT_ITEM_DISCOUNT_CENTS,
