...
import { uploadCsv, previewCsv, fetchProfessionalUsers } from '../api/migrationApi.js';
...
async function convertXlsxToCsv(file) {
  ...
  const csv = utils.sheet_to_csv(sheet, { FS: ',', strip: false });
    ...
    <div class="mig-grid">
      ...
      <template v-else-if="isFinancial">
        ...
        <div>
          ...
          <select v-model="bankAccountStrategy" class="mig-select">
            <option value="create">Criar conta dedicada "Importação Clinicorp"</option>
            <option value="existing">Usar conta bancária já cadastrada (1ª ativa)</option>
          </select>
          <div class="mig-help">
    ...
    <div v-if="preview" class="mig-preview">
      ...
      <div v-if="isFinancial && preview.reconciliation" class="mig-preview__reconciliation">
        ...
        <div v-if="preview.bank_account" class="mig-help" style="margin-top: 4px;">
          Conta bancária: <strong>{{ preview.bank_account.strategy === 'create' ? '"Importação Clinicorp"' : '1ª conta ativa' }}</strong>
      ...
      <div v-if="(isOperations || isFinancial) && preview.dentists && preview.dentists.length" class="mig-dentist-map">
        <h4 style="font-size: 13px; font-weight: 700; margin: 16px 0 6px;">
          Mapear dentistas → usuários ({{ preview.dentists.length }} distintos)
        </h4>
        <div class="mig-help" style="margin-bottom: 8px;">
          ...
          Mude o mapeamento e clique em <strong>Regerar pré-visualização</strong> pra ver o efeito.
        </div>
