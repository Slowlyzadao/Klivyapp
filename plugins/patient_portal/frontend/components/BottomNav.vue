...
const items = [
  ...
  { to: '/appointments', label: 'Consultas', icon: IconCalendar, match: ['appointments', 'appointment-detail'] },
  { to: '/health',       label: 'Saúde',     icon: IconHeart,    match: ['health'] },
  { to: '/financial',    label: 'Financeiro', icon: IconWallet,  match: ['financial'] },
  { to: '/more',         label: 'Mais',      icon: IconGrid,     match: ['more', 'profile', 'documents'] }
];
...
.pp-bottom-nav__item {
  ...
  color: var(--pp-color-text-muted); text-decoration: none;
  border-radius: 12px;
  transition: color 120ms ease, background 120ms ease, transform 80ms ease;
}
.pp-bottom-nav__item:active { transform: scale(0.95); }
.pp-bottom-nav__item--active {
  color: var(--pp-color-primary);
  background: rgba(37, 99, 235, 0.08);
}
.pp-bottom-nav__label { font-size: 11px; font-weight: 600; line-height: 1; }
</style>
