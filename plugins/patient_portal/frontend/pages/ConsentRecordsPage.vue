...
<template>
  <AppShell>
    <PageHeader
      title="Termos clínicos"
      back
      subtitle="Consentimentos para procedimentos"
      @back="$router.back()"
    />

    <div class="pp-consents">
      <div v-if="consents.loading" class="pp-consents__loading">
        Carregando…
      </div>
      ...
      <template v-else>
        ...
        <section v-if="consents.pending.length > 0">
          <h3
            class="pp-consents__section-title pp-consents__section-title--warn"
          >
          ...
          <ul class="pp-consents__list">
            <li v-for="c in consents.pending" :key="c.id">
              <router-link
                :to="{ name: 'consent-record-sign', params: { id: c.id } }"
                class="pp-consent-item pp-consent-item--pending"
              >
                <div class="pp-consent-item__icon">
                  <IconShield :size="20" />
                </div>
                <div class="pp-consent-item__body">
                  <div class="pp-consent-item__title">{{ c.title }}</div>
                  <div class="pp-consent-item__meta">
        ...
        <section v-if="consents.signed.length > 0">
          ...
          <ul class="pp-consents__list">
            <li v-for="c in consents.signed" :key="c.id">
              <router-link
                :to="{ name: 'consent-record-sign', params: { id: c.id } }"
                class="pp-consent-item"
              >
                <div class="pp-consent-item__icon pp-consent-item__icon--ok">
                  <IconCheck :size="20" />
                </div>
                <div class="pp-consent-item__body">
                  <div class="pp-consent-item__title">{{ c.title }}</div>
                  <div class="pp-consent-item__meta">
                    Assinado em {{ formatDate(c.signed_at) }}
                  </div>
                </div>
        ...
        <BaseCard
          v-if="consents.pending.length === 0 && consents.signed.length === 0"
        >
          <EmptyState
...
.pp-consents__section-title--warn {
  color: #92400e;
}
...
.pp-consent-item:hover {
  box-shadow: 0 4px 12px rgba(15, 23, 42, 0.06);
  border-color: var(--pp-color-primary);
}
.pp-consent-item:active {
  transform: scale(0.99);
}
.pp-consent-item--pending {
  border-color: #fde68a;
  background: #fffbeb;
}
...
.pp-consent-item__icon--ok {
  background: #d1fae5;
  color: #047857;
}

.pp-consent-item__body {
  flex: 1;
  min-width: 0;
}
.pp-consent-item__title {
  font-weight: 600;
  font-size: 14px;
  color: var(--pp-color-text);
}
.pp-consent-item__meta {
  font-size: 12px;
  color: var(--pp-color-text-muted);
  margin-top: 2px;
}
