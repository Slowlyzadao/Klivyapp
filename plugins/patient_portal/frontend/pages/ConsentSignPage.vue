...
function onSignatureChange(url) {
  ...
  <AppShell>
    ...
    <div v-else-if="consent" class="pp-consent-sign">
      ...
      <section class="pp-consent-sign__hero">
        ...
        <Badge
          :variant="consent.status === 'signed' ? 'success' : 'warning'"
          size="sm"
        >
          {{
      ...
      <BaseCard title="Conteúdo do termo">
        <p class="pp-consent-sign__body">
          {{ consent.body || 'Termo sem conteúdo descritivo.' }}
        </p>
      ...
      <BaseCard v-if="consent.can_sign" title="Sua assinatura">
        <SignaturePad ref="padRef" @update:data-url="onSignatureChange" />
        ...
        <div class="pp-consent-sign__done">
          ...
          <div>
            ...
            <div class="pp-consent-sign__done-meta">
              Em {{ formatDateTime(consent.signed_at) }} · canal:
              {{
                consent.signature_method === 'remote'
                  ? 'portal do paciente'
                  : 'tablet local'
              }}
            </div>
      ...
      <p
        v-if="consent.expires_at && consent.status !== 'signed'"
        class="pp-consent-sign__expires"
      >
    ...
    >
      ...
      <template #action>
        <BaseButton @click="$router.push({ name: 'consent-records' })">
          Voltar
        </BaseButton>
      </template>
    </EmptyState>
...
.pp-consent-sign__agree input[type='checkbox'] {
  margin-top: 2px;
  flex-shrink: 0;
}

.pp-consent-sign__error {
  color: #b91c1c;
  font-size: 13px;
  margin: 0 0 12px;
}
.pp-consent-sign__expires {
  text-align: center;
  font-size: 12px;
  color: var(--pp-color-text-muted);
  margin: 0;
...
.pp-consent-sign__done-title {
  font-size: 16px;
  font-weight: 700;
  color: var(--pp-color-text);
}
.pp-consent-sign__done-meta {
  font-size: 13px;
  color: var(--pp-color-text-muted);
  margin-top: 2px;
}
