...
const pendingDocRequests = computed(() =>
  docs.requests.filter(r => r.status === 'pending')
);
...
function labelType(t) {
  return TYPE_LABELS[t] || t || 'Documento';
}
    ...
    <div class="pp-health">
      ...
      <section v-if="consents.pendingCount > 0">
        <router-link
          ...
          class="pp-health__pending-banner"
        >
          ...
          <div class="pp-health__pending-body">
            <div class="pp-health__pending-title">
              {{ consents.pendingCount }} termo{{
                consents.pendingCount > 1 ? 's' : ''
              }}
              aguardando sua assinatura
            </div>
            <div class="pp-health__pending-text">
      ...
      <section>
        <header class="pp-health__section-head">
          ...
          <router-link
            :to="{ name: 'document-request' }"
            class="pp-health__cta"
          >
            + 2ª via
          </router-link>
        </header>
        ...
        <div v-if="pendingDocRequests.length > 0" class="pp-health__pending">
          ...
          <ul class="pp-health__pending-list">
            <li
              v-for="req in pendingDocRequests"
              :key="req.id"
              class="pp-health__pending-item"
            >
              <div>
      ...
      <section v-if="anamneses.exposed && anamneses.items.length > 0">
        ...
        <ul class="pp-health__anam-list">
          <li v-for="a in anamneses.items" :key="a.id">
            <router-link
              :to="{ name: 'anamnesis-detail', params: { id: a.id } }"
              class="pp-health__anam-item"
            >
              <div class="pp-health__anam-icon"><IconHeart :size="20" /></div>
              <div class="pp-health__anam-body">
                <div class="pp-health__anam-title">
                  {{ a.specialty || 'Anamnese geral' }} · v{{
                    a.version_number
                  }}
                </div>
                <div class="pp-health__anam-meta">
                  {{ formatDate(a.finalized_at)
                  }}<span v-if="a.professional">
                    · {{ a.professional.name }}</span
                  >
                </div>
              </div>
              <IconChevronRight :size="18" />
...
.pp-health__pending-banner:hover {
  box-shadow: 0 4px 12px rgba(15, 23, 42, 0.06);
}
.pp-health__pending-banner:active {
...
.pp-health__pending-body {
  flex: 1;
  min-width: 0;
}
.pp-health__pending-title {
  font-weight: 700;
  font-size: 14px;
  color: #78350f;
}
.pp-health__pending-text {
  font-size: 12px;
  color: #92400e;
  margin-top: 2px;
}

.pp-health__section-head {
  display: flex;
...
.pp-health__pending-line {
  font-size: 13px;
  font-weight: 600;
  color: var(--pp-color-text);
}
.pp-health__pending-notes {
  font-size: 12px;
  color: var(--pp-color-text-muted);
  margin-top: 2px;
}
...
.pp-health__anam-item:hover {
  box-shadow: 0 4px 12px rgba(15, 23, 42, 0.06);
  border-color: var(--pp-color-primary);
}
.pp-health__anam-item:active {
  transform: scale(0.99);
}
...
.pp-health__anam-body {
  flex: 1;
  min-width: 0;
}
.pp-health__anam-title {
  font-weight: 600;
  font-size: 14px;
  color: var(--pp-color-text);
}
.pp-health__anam-meta {
  font-size: 12px;
  color: var(--pp-color-text-muted);
  margin-top: 2px;
}
