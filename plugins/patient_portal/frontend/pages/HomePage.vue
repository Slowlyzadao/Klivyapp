...
const pendingConsents = computed(
  () => home.value?.cards?.pending_consents_count || 0
);
const hasOverdue = computed(
  () => (financial.value?.overdue_amount_cents || 0) > 0
);
const overdueRestriction = computed(
  () => home.value?.cards?.overdue_restriction || {}
);
...
<template>
  <AppShell>
    <div class="pp-home">
      ...
      <section v-if="!loading" class="pp-home__hero">
        ...
        <template v-if="next">
          ...
          <p
            v-if="next.professional || next.service"
            class="pp-home__hero-detail"
          >
            {{
              [next.professional?.name, next.service?.name]
                .filter(Boolean)
                .join(' · ')
            }}
          </p>
          ...
          <router-link
            v-else
            :to="{ name: 'appointment-detail', params: { id: next.id } }"
            class="pp-home__hero-cta"
          >
            Ver detalhes
          </router-link>
        </template>
        <template v-else>
          ...
          <p class="pp-home__hero-detail">
            Aqui você acompanha consultas, documentos, financeiro e fala com a
            clínica.
          </p>
          <router-link
      ...
      <section v-if="pendingConsents > 0" class="pp-home__section">
        <router-link
          :to="{ name: 'consent-records' }"
          class="pp-home__alert pp-home__alert--warn"
        >
          <IconShield :size="20" />
          <div>
            <div class="pp-home__alert-title">
              {{ pendingConsents }} termo{{
                pendingConsents > 1 ? 's' : ''
              }}
              pendente{{ pendingConsents > 1 ? 's' : '' }}
            </div>
            <div class="pp-home__alert-text">
              Revise e assine para liberar seus procedimentos.
            </div>
          </div>
      ...
      <section v-if="hasOverdue" class="pp-home__section">
        <router-link
          :to="{ name: 'financial' }"
          class="pp-home__alert pp-home__alert--danger"
        >
          <IconWallet :size="20" />
          <div>
            <div class="pp-home__alert-title">
              {{ formatCurrency(financial?.overdue_amount_cents || 0) }} em
              parcelas vencidas
            </div>
            <div class="pp-home__alert-text">
      ...
      <section v-if="home?.cards?.recall_due" class="pp-home__section">
        <BaseCard>
          <div class="pp-home__recall">
            <IconHeart :size="32" />
            <div>
              <div class="pp-home__recall-title">
                Faz tempo desde sua última consulta
              </div>
              <div class="pp-home__recall-text">
                Que tal agendar um retorno? Sua clínica está com horários
                disponíveis.
              </div>
            </div>
            <div class="pp-home__recall-actions">
              <button
                type="button"
                class="pp-home__recall-dismiss"
                :disabled="dismissing"
                @click="onDismissRecall"
              >
                Depois
              </button>
              <router-link
                :to="{ name: 'appointment-new' }"
                class="pp-home__recall-cta"
                @click="onScheduleRecall"
              >
                Agendar
              </router-link>
            </div>
      ...
      <section class="pp-home__section">
        ...
        <div class="pp-home__stats">
          <router-link
            :to="{ name: 'financial' }"
            class="pp-home__stat"
            :class="{ 'pp-home__stat--danger': hasOverdue }"
          >
            <div class="pp-home__stat-label">Em aberto</div>
            <div class="pp-home__stat-value">
              {{ formatCurrency(financial?.open_amount_cents || 0) }}
            </div>
            <div class="pp-home__stat-hint">
              {{ financial?.open_count || 0 }} parcela{{
                (financial?.open_count || 0) !== 1 ? 's' : ''
              }}
            </div>
          </router-link>
          <router-link :to="{ name: 'health' }" class="pp-home__stat">
            ...
            <div class="pp-home__stat-value">
              {{ home?.cards?.documents_recent?.length || 0 }}
            </div>
          ...
          <div class="pp-home__stat">
            ...
            <div class="pp-home__stat-value">
              {{ home?.cards?.unread_messages_count || 0 }}
            </div>
      ...
      <p class="pp-home__footer">Portal versão Sprint D · v0.4</p>
    </div>
  </AppShell>
</template>
...
<style scoped>
.pp-home {
  padding: 4px var(--pp-content-pad-x) 32px;
}
...
.pp-home__hero-detail:not(:first-of-type) {
  text-transform: none;
}
.pp-home__hero-cta {
  position: relative;
...
/* Quick grid — 3 cols (mobile) → 4 cols (tablet) → 6 cols (desktop) */
.pp-home__quick {
  display: grid;
  gap: 10px;
...
.pp-home__quick-item:hover {
  box-shadow: 0 4px 12px rgba(15, 23, 42, 0.06);
  border-color: var(--accent);
}
.pp-home__quick-item:active {
...
.pp-home__alert:hover {
  box-shadow: 0 4px 12px rgba(15, 23, 42, 0.06);
}
.pp-home__alert:active {
...
.pp-home__alert--danger {
  background: linear-gradient(135deg, #fef2f2 0%, #fee2e2 100%);
  border: 1px solid #fca5a5;
  color: #7f1d1d;
}
.pp-home__alert > svg:first-child {
  flex-shrink: 0;
}
.pp-home__alert > div {
  flex: 1;
...
.pp-home__recall > svg:first-child {
  color: #ef4444;
  flex-shrink: 0;
}
.pp-home__recall > div:nth-child(2) {
  flex: 1;
  min-width: 0;
}
.pp-home__recall-title {
  font-weight: 700;
  font-size: 15px;
  color: var(--pp-color-text);
}
.pp-home__recall-text {
  font-size: 13px;
  color: var(--pp-color-text-muted);
  margin-top: 2px;
}
.pp-home__recall-actions {
  display: flex;
  gap: 8px;
  flex-shrink: 0;
}
...
.pp-home__recall-dismiss:hover:not(:disabled) {
  background: #f1f5f9;
}
.pp-home__recall-dismiss:disabled {
  opacity: 0.5;
  cursor: not-allowed;
}
.pp-home__recall-cta {
...
/* Stats — 3 cols (mobile/tablet) → 3 cols mais largas no desktop */
.pp-home__stats {
  display: grid;
  grid-template-columns: repeat(3, 1fr);
  gap: 8px;
