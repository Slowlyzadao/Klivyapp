...
function iconFor(kind) {
  return (KIND_MAP[kind] || KIND_MAP.generic).icon;
}
...
<template>
  <AppShell>
    ...
    <div class="pp-notifs">
      <div v-if="notifications.loading" class="pp-notifs__loading">
        Carregando…
      </div>
...
.pp-notifs__item:hover {
  border-color: var(--pp-color-primary);
}
.pp-notifs__item:active {
  transform: scale(0.99);
}
.pp-notifs__item--unread {
  background: #fafbff;
