class Api::V1::Accounts::PatientsController < Api::V1::Accounts::BaseController
  ...
  def index
    ...
    @overdue_cents = preload_overdue_cents(patient_ids)
  ...
  def filter_patients(scope)
    ...
    apply_financial_filter(scope, params[:financial_status])
  end

  # Filtro por situação financeira (canon: "inadimplente" = tem parcela VENCIDA).
  #
  # Inadimplente = paciente com >= 1 parcela em aberto (pendente|parcial|vencido)
  # já vencida (`due_date < hoje`) e com saldo > 0. Usamos `due_date` como
  # verdade — não só `status: vencido` — porque o status `vencido` é carimbado
  # por um job diário; entre o vencimento e o job, a parcela ainda está
  # `pendente`/`parcial` mas já está atrasada de fato.
  #
  # Adimplente = NÃO inadimplente (inclui quem só tem parcela a vencer ou está
  # tudo quitado). `where.not(patient_id: nil)` blinda o `NOT IN` contra o
  # gotcha do Postgres (subquery com NULL zera o resultado).
  def apply_financial_filter(scope, value)
    return scope if value.blank?

    overdue_patient_ids = Financial::Installment
                          .where(account_id: Current.account.id, status: %w[pendente parcial vencido])
                          .where.not(patient_id: nil)
                          .where('due_date < ?', Date.current)
                          .group(:patient_id)
                          .having('SUM(amount_cents - received_amount_cents) > 0')
                          .select(:patient_id)

    case value
    when 'inadimplente' then scope.where(id: overdue_patient_ids)
    when 'adimplente'   then scope.where.not(id: overdue_patient_ids)
    else scope
    end
  end
  ...
  # Porção VENCIDA do saldo em aberto (due_date < hoje) — mesma definição do
  # card "Devedor (Vencido)" do prontuário e do filtro de inadimplência. O
  # saldo total (`balance_due_cents`) menos isto = parcelas a vencer. Serve pro
  # tooltip da coluna SALDO reconciliar com a aba financeira do paciente.
  def preload_overdue_cents(patient_ids)
    return {} if patient_ids.empty?
