...
// - `hint` mostra o preço SÓ quando > 0 — serviços sem preço (R$ 0,00) não
//   poluem a lista com valor irrelevante. Busca interna do FormSelect inclui
//   `badge` no haystack, então digitar o ID encontra a opção.
const serviceOptions = computed(() =>
  (props.agendaServices || []).map(svc => {
    ...
    return {
      ...
      label: svc.name,
      badge: svc.id,
      hint: price > 0 ? formatCurrency(svc.price) : '',
    };
  })
);
