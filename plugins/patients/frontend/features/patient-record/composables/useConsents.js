...
export function useConsents(patientIdRef) {
  ...
  const create = async payload => {
    // No caminho via modelo (document_template_id) o tipo vem do template —
    // exigir só o título; senão, mantém a regra legada (tipo + título).
    const hasTemplate = Boolean(payload?.document_template_id);
    if ((!hasTemplate && !payload?.document_type) || !payload?.title) {
