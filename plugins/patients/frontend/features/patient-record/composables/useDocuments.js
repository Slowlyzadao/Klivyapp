...
export function useDocuments(patientIdRef) {
  ...
  const generate = async form => {
    ...
    try {
      ...
      // Ponte com o editor de modelos: com template_id o backend delega pro
      // DocumentTemplates::PdfGenerator (Grover) e ignora document_type. O
      // título cai pro nome do modelo no servidor quando vazio.
      if (form.document_template_id) {
        payload.document_template_id = form.document_template_id;
        payload.title = form.title || payload.title;
      }
      // Campos de preenchimento do modelo (input.*) digitados no modal.
      if (form.inputs && Object.keys(form.inputs).length) {
