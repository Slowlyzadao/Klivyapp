...
export function useFinancialActions(
  patientIdRef,
  { onSuccess, onSetupRequired } = {},
) {
  const { t } = useI18n();

  // Setup financeiro incompleto: o backend (ensure_setup_complete!) bloqueia
  // QUALQUER escrita v2 com 412 financial_setup_required enquanto Plano de
  // Contas + Contas Bancárias + Formas de Pagamento não estiverem prontos. Em
  // vez de um erro genérico ("Erro ao criar orçamento"), sinaliza pro componente
  // abrir o modal premium de configuração (com CTA pro assistente). Retorna
  // true se já tratou o erro.
  const notifyIfSetupRequired = error => {
    if (!isFinancialSetupRequired(error)) return false;
    if (typeof onSetupRequired === 'function') onSetupRequired();
    return true;
  };
  ...
  const pay = async (txId, payload) => {
    ...
    try {
      ...
      await FinancialV2.installments.pay(txId, {
        // Refactor 2026-05-24: payment_method_id + tipo/value pros
        // modificadores. Backend recalcula cents do gross × pct quando
        // type='percent' (anti-tamper). Compat com cents legacy preservada.
        ...
        interest_value: payload?.interest_value,
        interest_cents: payload?.interest_cents,
        fine_type: payload?.fine_type,
        fine_value: payload?.fine_value,
  ...
  const refund = async (txId, payload = {}) => {
    try {
      ...
      await FinancialV2.installments.refund(txId, {
        amount_cents: payload?.amount_cents,
        // Refactor 2026-05-24: payment_method_id (canon) + payment_method
        // (kind string legacy) — backend usa _id quando vier.
        payment_method_id: payload?.payment_method_id,
        payment_method: payload?.payment_method,
        notes: payload?.notes,
  ...
  const approveEstimate = async (estimateId, opts = {}) => {
    try {
      ...
      // `opts.silent` evita o toast — útil no fast-path à vista que vai mostrar
      // o próprio toast contextual.
      const res = await FinancialV2.budgets.approve(estimateId, {});
      if (!opts.silent) {
      ...
      if (notifyIfSetupRequired(error)) return { ok: false };
      // eslint-disable-next-line no-console
  ...
  const editInstallment = async ({ budgetId, installmentId, payload }) => {
    ...
    try {
      ...
      // Refactor 2026-05-24: repassa `payment_method_id` (canon) — backend
      // pode persistir além do kind string pra rastreabilidade.
  ...
  const buildBudgetPayloadFromLegacy = (legacy, patientId) => {
    ...
    const budget = {
      ...
      // string (legacy compat). Backend pode gravar `payment_method_id` no
      // Budget pra rastreabilidade quando vier — fallback pra string.
      payment_method_id: legacy.payment_method_id || null,
  ...
  // Fast-path "À vista" (2026-05-28): cria Budget(rascunho) + aprova
  // imediatamente em uma única operação. Resultado: orçamento aparece em
  // A Receber pronto pra ser pago. Pula o passo "Precisa aprovação" do
  // fluxo padrão (que faz sentido pra parcelado, mas é fricção pra avulso).
  //
  // Decisão: NÃO abre Receber Pagamento automaticamente — o operador
  // clica "Receber" quando o dinheiro estiver na mão (mantém regra dura
  // [[feedback-status-no-false-positive]] de não marcar pago sem operador).
  const createAndApproveAvulso = async payload => {
    estimateLoading.value = true;
    try {
      ...
      if (!budgetId) return { ok: false };

      const approved = await approveEstimate(budgetId, { silent: true });
      if (!approved.ok) return { ok: false };

      useNotification.success(
        'Lançamento criado e aprovado. Disponível em A Receber.',
      );
      return { ok: true, data: approved.data, budgetId };
    } finally {
      estimateLoading.value = false;
    }
  };
