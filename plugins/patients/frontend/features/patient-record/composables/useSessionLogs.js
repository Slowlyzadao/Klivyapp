...
export function useSessionLogs(patientIdRef) {
  ...
  // PR audit 2026-05-21: API `session_logs_controller.rb:28` tem default
  // Kaminari per_page=20 que escondia silenciosamente entradas após a 20a.
  // Streit conta #12274 tinha pacientes com 30+ SessionLogs importados —
  // só os 20 mais recentes apareciam na aba "Ficha Clinica", os outros
  // ficavam invisiveis pela UI (nenhum botao "carregar mais" / paginacao
  // visivel). Pedimos per_page=500 explicito como medida pragmatica
  // (paciente com 500+ SessionLogs e improvavel no curto prazo; se virar
  // caso real, migra pra paginacao visivel com infinite scroll).
  const DEFAULT_PER_PAGE = 500;

  const fetch = async (params = {}) => {
    isLoading.value = true;
    try {
      const requestParams = { per_page: DEFAULT_PER_PAGE, ...params };
      const { data } = await SessionLogsAPI.get(resolveId(), requestParams);
  ...
  const filter = async filterValues => {
    const params = { per_page: DEFAULT_PER_PAGE };
