...
// "Cobrar" no header do prontuário é uma ação financeira DO PACIENTE →
// gateada por `patients.manage_financial` (mesma fonte de verdade da aba),
// não por `financial.create_transaction` (que gateia o módulo standalone).
const canCharge = computed(() => can('patients', 'manage_financial'));
...
const allTabs = [
  ...
  { id: 'consents', label: 'Consentimentos', icon: 'i-lucide-pen-tool' },
  {
    ...
    // Gate da aba = RBAC do PACIENTE, não do módulo financeiro standalone.
    // `patients.view_financial` ("Ver financeiro do paciente") é a fonte de
    // verdade única — mesma key do backend (PatientSummaries/Timelines) e dos
    // presets (Especialista/Gerente têm; Recepção/SDR não). A key antiga
    // `financial.view_transactions` NÃO existe no catálogo (KlivyRole::
    // PermissionsCatalog) → `can()` retornava sempre false e a aba sumia pra
    // todo não-admin. Ver CHANGELOG 1.8.0.43.
...
// Guard de deep-link (?tab=…): se a URL pede uma aba que o usuário não pode
// ver (ex.: ?tab=financial sem `patients.view_financial`, ?tab=audit sem
// `patients.view_audit`), cai pra 'general'. Complementa o filtro da nav —
// sem isso, forçar `?tab=financial` na URL montava o componente da aba (o
// `v-else-if` no template só checa `activeTab`, não a permissão). O backend
// já devolve 403, mas a aba não deve nem renderizar o shell.
//
// IMPORTANTE: só age depois das permissões carregarem (`isLoaded`). Antes
// disso `can()` retorna false pra tudo — redirecionar aqui expulsaria um
// usuário AUTORIZADO da aba antes do fetch das perms resolver. Por isso é
// reativo (watch em `isLoaded`+`activeTab`), não imperativo no onMounted.
// Admin/super_admin passam pelo bypass do `can()` → todas as abas em
// `tabs.value` → nunca caem aqui.
const ensureActiveTabAllowed = () => {
  if (!isLoaded.value) return;
  const allowed = tabs.value.some(tab => tab.id === activeTab.value);
  if (!allowed) activeTab.value = 'general';
};

watch([isLoaded, activeTab], ensureActiveTabAllowed);
