...
import RecurringBillingsSection from '@plugins/patients/frontend/features/patient-record/components/financial-tab/RecurringBillingsSection.vue';
// F3 do PaymentPlanWizardV2: wizard de aprovação financeira atrás de feature flag
// `payment_plan_wizard_v2`. Quando ON, substitui o handler antigo (`approveEstimate`
// direto) por abertura do wizard. Cancelar orçamento continua botão separado.
import PaymentPlanWizardV2 from '@plugins/financial/frontend/features/financial/v2/components/wizard/PaymentPlanWizardV2.vue';
...
// F3 — Gate do PaymentPlanWizardV2. Quando ON, o clique em "Aprovar orçamento"
// (em entries com `source_type='estimate'` em rascunho) abre o wizard 5-step
// em vez de chamar `approveEstimate(id)` direto. "Cancelar orçamento" continua
// como botão separado (decisão UX confirmada — não muda).
// Default OFF — operador ativa por conta via Super Admin / runner.
const isPaymentPlanWizardEnabled = computed(() => {
  if (!accountId.value) return false;
  return store.getters['accounts/isBetaFeatureEnabledOnAccount'](
    accountId.value,
...
// Estado do wizard — carrega o budget completo on-demand antes de abrir
// (wizard precisa de `budget.items[]` que não vem na timeline agregada).
const wizardOpen = ref(false);
const wizardBudget = ref(null);
...
// Modais "Novo Lançamento" + setup incompleto. Declarados antes de
// useFinancialActions porque o callback onSetupRequired os referencia: ao
// receber 412 financial_setup_required, fecha o form e abre o modal premium.
...
const {
  ...
  uploadProof,
  createEstimate,
  createAndApproveAvulso,
  editInstallment,
} = useFinancialActions(patientId, {
  onSuccess: reloadAll,
  onSetupRequired: () => {
...
// Modal premium de setup incompleto → leva ao assistente de configuração
// (rota canon financial_v2_setup). Fecha o modal antes de navegar.
const goToFinancialSetup = () => {
  showSetupModal.value = false;
  router
    .push({ name: 'financial_v2_setup', params: { accountId: accountId.value } })
    .catch(() => {});
};

const handleEstimateConfirm = async payload => {
  // Fast-path à vista (2026-05-28): cria + aprova num passo só, sem wizard.
  if (payload.recurrence_type === 'avulso') {
    const { ok } = await createAndApproveAvulso(payload);
    if (ok) showEstimateModal.value = false;
    return;
  }

  // Parcelado: cria rascunho + abre wizard DIRETO (pula o "Precisa aprovação"
  // intermediário). Se o wizard estiver OFF (feature flag), comportamento
  // legacy persiste — fica em "Precisa aprovação" pro operador clicar Aprovar.
  const created = await createEstimate(payload);
  if (!created.ok) return;
  showEstimateModal.value = false;

  const budgetId = created.data?.data?.id || created.data?.id;
  if (!budgetId) return;

  if (isPaymentPlanWizardEnabled.value) {
    wizardLoading.value = true;
    try {
      const { data } = await FinancialV2.budgets.show(budgetId);
      wizardBudget.value = data?.data || data;
      wizardOpen.value = true;
    } catch (e) {
      // eslint-disable-next-line no-console
      console.error('[Financial] Falha ao abrir wizard pós-criação', e);
    } finally {
...
const onTimelineApproveEstimate = async entry => {
  // F3 — Quando flag ON, abre wizard. Quando OFF, mantém comportamento legacy
  // (chamada direta ao endpoint, que distribui em N parcelas iguais com forma
  // única). Carrega o budget completo (com items[]) antes de abrir o wizard.
  if (isPaymentPlanWizardEnabled.value) {
    wizardLoading.value = true;
    try {
      const { data } = await FinancialV2.budgets.show(entry.source_id);
      wizardBudget.value = data?.data || data;
      wizardOpen.value = true;
    } catch (error) {
      // eslint-disable-next-line no-console
      console.error('[Financial] Falha ao carregar orçamento pro wizard', error);
      useNotification.error('Não foi possível carregar o orçamento.');
    } finally {
      wizardLoading.value = false;
    }
    return;
  }
  // Comportamento legacy — sem flag
  await approveEstimate(entry.source_id);
};

// Handler executado quando o wizard confirma a aprovação. Refresh dos
// dados (timeline + summary) pra refletir as parcelas geradas.
const onWizardApproved = async () => {
  wizardOpen.value = false;
  wizardBudget.value = null;
...
<template>
  <div class="tab-pane fade-in">
    ...
    <!-- Mensalidades fixas (decisão 2026-05-28): contrato recorrente
         (Financial::RecurringBilling) que gera Budget+Installment a cada
         período. Lista colapsável; default expandida se houver billings,
         colapsada e mostrando só o botão Nova se vazio. -->
    <RecurringBillingsSection :patient-id="patientId" />
    ...
    <!-- Modal premium (beclinic_core) quando o módulo financeiro ainda não foi
         configurado — substitui o toast genérico. Tom 'info' (azul, não
         destrutivo) + CTA pro assistente de configuração. -->
    <ConfirmDangerModal
      v-model:show="showSetupModal"
      tone="info"
      icon="i-lucide-list-checks"
      :title="t('PATIENT_FINANCIAL.MESSAGES.SETUP_REQUIRED_TITLE')"
      :message="t('PATIENT_FINANCIAL.MESSAGES.SETUP_REQUIRED')"
      :confirm-label="t('PATIENT_FINANCIAL.MESSAGES.SETUP_REQUIRED_CTA')"
      @confirm="goToFinancialSetup"
    />
    ...
    <!-- F3 — Wizard de aprovação financeira. Aberto via handler
         `onTimelineApproveEstimate` quando feature flag `payment_plan_wizard_v2`
         está ON. Caso contrário, fluxo legacy permanece (botão "Aprovar
         orçamento" chama `approveEstimate(id)` direto, distribuindo em N parcelas
         iguais com forma única). -->
    <PaymentPlanWizardV2
      :show="wizardOpen"
      :budget="wizardBudget"
      @close="onWizardClose"
      @approved="onWizardApproved"
    />
  </div>
</template>
