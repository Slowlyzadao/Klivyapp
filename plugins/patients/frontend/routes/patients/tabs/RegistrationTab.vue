...
// Snapshot do contato carregado — base do grandfathering da validação: só
// valida e-mail/telefone se o operador mudar o valor (não trava save de
// registro legado com dado inválido intocado). Re-snapshot ao trocar de paciente.
const originalContact = ref({ email: '', phone: '', altPhone: '' });
watch(
  () => props.patient?.id,
  () => {
    originalContact.value = {
      ...
      altPhone: props.patient?.emergency_contact?.phone || '',
    };
  },
  { immediate: true }
);
...
const handleSave = () => {
  ...
  saveRegistration(props.patient, { original: originalContact.value });
};
