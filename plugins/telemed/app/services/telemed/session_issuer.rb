...
module Telemed
  class SessionIssuer
    ...
    # URL que o BROWSER usa pra conectar no LiveKit (vai no token). Em dev, o
    # servidor LiveKit roda num container publicado no host: o Rails (dentro de
    # container) o alcança via `host.docker.internal`, mas o browser (no host)
    # só via `localhost`. `LIVEKIT_PUBLIC_URL` separa os dois — server-side
    # (admit/gravação) continua usando `@creds.url` (host.docker.internal). Em
    # prod (LiveKit Cloud) os dois são o mesmo `wss://` e a env fica vazia,
    # caindo no `@creds.url`.
    def public_url
      ENV['LIVEKIT_PUBLIC_URL'].presence || @creds.url
    end
