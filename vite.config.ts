...
// Prevent infinite page-reload loop caused by ~2300 i18n locale JSONs
// during Vite warmup. Without this, each JSON triggers a full page reload,
// and the browser never finishes loading.
const i18nHmrPlugin = {
  name: 'i18n-hmr-silent',
  handleHotUpdate({ file, server }) {
    if (file.includes('/i18n/locale/') && file.endsWith('.json')) {
      // Silently accept — don't trigger a full page reload
      return [];
    }
  },
};

let plugins = [ruby(), vue(vueOptions), i18nHmrPlugin];
...
export default defineConfig({
  server: {
    ...
    watch: {
      usePolling: true,
      interval: 1000,
      // Evita travar o watcher exaurindo file descriptors ou memória
